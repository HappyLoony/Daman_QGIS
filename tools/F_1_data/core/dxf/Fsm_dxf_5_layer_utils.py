...
class DxfLayerUtils:
    ...
    def should_use_blocks_for_layer(self, full_name: str, force_blocks: bool = False) -> bool:
        ...
        - Нарезка ЗПР (для П_ОЗУ): слои Le_2_1_, Le_2_2_, Le_2_5_, Le_2_6_, Le_2_7_ (полигоны)
          НО НЕ для точек (Le_2_8_) - они экспортируются как простая геометрия
        ...
        # НО НЕ слои точек (Le_2_8_) - они остаются простой геометрией
        pozu_polygon_prefixes = ['Le_2_1_', 'Le_2_2_', 'Le_2_5_', 'Le_2_6_', 'Le_2_7_']
    ...
    def add_linetype(self, doc, linetype_name: str):
        ...
        try:
            if linetype_name == 'DASHED':
                doc.linetypes.add(
                    name='DASHED',
                    # Формат ezdxf: [total_pattern_length, elem1, elem2, ...]
                    # elem > 0 = линия, elem < 0 = пробел, elem == 0.0 = точка
                    pattern=[0.75, 0.5, -0.25],  # total=0.75, линия 0.5, пробел 0.25
                ...
                doc.linetypes.add(
                    name='DASHDOT',
                    # Формат ezdxf: [total_pattern_length, elem1, elem2, ...]
                    pattern=[1.0, 0.5, -0.25, 0.0, -0.25],  # total=1.0, линия 0.5, пробел 0.25, точка, пробел 0.25
                    description='Штрихпунктирная __ . __ . __'
                )
