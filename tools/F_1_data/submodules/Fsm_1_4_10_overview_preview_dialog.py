# -*- coding: utf-8 -*-
"""
Диалог предпросмотра обзорной карты с выбором варианта отображения
Позволяет выбрать оптимальный масштаб и DPI для обзорной карты перед экспортом
"""
...
class Fsm_1_4_10_OverviewPreviewDialog(BaseResponsiveDialog):
    ...
    # scale_factor - множитель относительно базового масштаба из метаданных
    # Названия формируются динамически на основе current_scale * scale_factor
    # Диапазон от x0.5 (крупнее) до x200 (мельче) для разных размеров территорий
    SCALE_FACTORS = [
        0.5, 1.0, 1.5, 2.0,
        3.0, 4.0, 5.0, 7.5,
        10.0, 15.0, 20.0, 50.0,
    ...
    DEFAULT_VARIANT_IDX = 1  # x1.0 (базовый масштаб)

    def __init__(self, layout: QgsPrintLayout, current_scale: float, parent=None):
        """Инициализация диалога

        Args:
            layout: Макет QGIS с overview_map
            current_scale: Текущий масштаб обзорной карты
    ...
    def _setup_network_cache(self):
        ...
        try:
            ...
            if cache:
                ...
                log_info(f"Fsm_1_4_10: Сетевой кэш QGIS: {cache_size_mb:.1f} / {max_size_mb:.1f} MB")
            else:
                log_warning("Fsm_1_4_10: Сетевой кэш QGIS не настроен")
        except Exception as e:
            log_warning(f"Fsm_1_4_10: Ошибка настройки сетевого кэша: {e}")
    ...
    def _init_ui(self):
        ...
        # Заголовок
        title_label = QLabel("Выберите масштаб обзорной карты:")
    ...
    def _render_previews(self):
        ...
        """
        # Находим overview_map в макете
        overview_map = self._get_overview_map()
        if not overview_map:
            log_warning("Fsm_1_4_10: overview_map не найден в макете")
        ...
        original_scale = overview_map.scale()

        # DPI 300 - требование Приказа Росреестра П/0148 (экспорт)
        preview_dpi = self.DPI

        log_info(f"Fsm_1_4_10: Начало рендеринга {len(self.SCALE_FACTORS)} превью с DPI={preview_dpi}")
        ...
        overview_map.setScale(mid_scale)
        ...
        if sip.isdeleted(overview_map):
            log_warning("Fsm_1_4_10: overview_map удалён во время предзагрузки тайлов")
        ...
        for idx, scale_factor in enumerate(self.SCALE_FACTORS):
            ...
            if self._cancelled:
                log_info(
                    f"Fsm_1_4_10: Рендер прерван пользователем на варианте "
                    f"{idx}/{len(self.SCALE_FACTORS)}"
                )
                break

            # Проверяем что C++ объект overview_map ещё существует
            # QEventLoop может обработать события, удаляющие макет
            if sip.isdeleted(overview_map):
                log_warning("Fsm_1_4_10: overview_map удалён во время рендеринга, прерываем")
                break
            ...
            try:
                ...
                overview_map.setScale(new_scale)
                overview_map.refresh()
                ...
                pixmap = self._render_map_preview(overview_map, preview_dpi)
                ...
                if "deleted" in str(e).lower():
                    log_warning(f"Fsm_1_4_10: overview_map удалён во время рендеринга варианта {idx}")
                    break
                log_error(f"Fsm_1_4_10: Ошибка рендеринга варианта {idx}: {e}")
                self.preview_labels[idx].setText("Ошибка")
            except Exception as e:
                log_error(f"Fsm_1_4_10: Ошибка рендеринга варианта {idx}: {e}")
                self.preview_labels[idx].setText("Ошибка")
        ...
        if not sip.isdeleted(overview_map):
            overview_map.setScale(original_scale)
            overview_map.refresh()
        ...
        try:
            ...
            if cache:
                cache_size_mb = cache.cacheSize() / (1024 * 1024)
                log_info(f"Fsm_1_4_10: Превью отрендерены. Сетевой кэш: {cache_size_mb:.1f} MB")
            else:
                log_info("Fsm_1_4_10: Превью вариантов отрендерены")
        except Exception:
            log_info("Fsm_1_4_10: Превью вариантов отрендерены")
    ...
    def _get_overview_map(self) -> Optional[QgsLayoutItemMap]:
        """Получение элемента overview_map из макета
    ...
    def _render_map_preview(self, map_item: QgsLayoutItemMap, dpi: int) -> Optional[QPixmap]:
        ...
        try:
            ...
            if sip.isdeleted(map_item):
                log_warning("Fsm_1_4_10: map_item удалён, пропускаем рендеринг")
                return None
            ...
            if image.isNull():
                log_warning("Fsm_1_4_10: Рендеринг вернул пустое изображение")
                return None
            ...
            log_error(f"Fsm_1_4_10: Ошибка рендеринга превью: {e}")
            return None
    ...
    def accept(self):
        ...
        log_info(f"Fsm_1_4_10: Выбран вариант DPI={dpi}, scale_factor={scale_factor}")
