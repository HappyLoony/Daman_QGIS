...
class Fsm_2_1_8_SelectionEngine:
    ...
    def perform_selection_zu(self, boundaries_exact: QgsVectorLayer, boundaries_10m: QgsVectorLayer,
                            boundaries_minus2cm: QgsVectorLayer, boundaries_500m: QgsVectorLayer,
                            source_layer: QgsVectorLayer,
                            object_type: str = 'ZU',
                            progress_callback: Optional[Callable[[int, int], None]] = None
                            ) -> Tuple[Optional[QgsVectorLayer], Optional[QgsVectorLayer], Optional[QgsVectorLayer]]:
        ...
        Args:
            ...
            boundaries_500m: Слой границ с буфером 500м (L_1_1_3)
            source_layer: Исходный слой (ЗУ)
            object_type: Тип объекта ('ZU')

        Returns:
            tuple: (final_layer_1, final_layer_2, final_layer_3) или (None, None, None) при ошибке
        """
        try:
            ...
            if boundaries_500m.featureCount() == 0:
                raise ValueError(f"Слой '{boundaries_500m.name()}' не содержит объектов. Сначала импортируйте границы через F_1_1.")

            # Получаем объединенные геометрии для слоёв границ
            ...
            # Создаем геометрию 500м и буфер -2см
            boundaries_geom_500m = self.geo_processor.get_boundaries_geometry(boundaries_500m)
            if not boundaries_geom_500m:
            ...
            result_layer_3 = self.layer_builder.create_result_layer(source_layer, layer_name=LAYER_SELECTION_ZU_500M, object_type=object_type, target_crs=target_crs)
            if not result_layer_3:
            ...
            for feature in source_layer.getFeatures():
                ...
                if intersects_500m:
                    ...
                    new_feature_3.setGeometry(QgsGeometry(final_geom))
                    self.attr_mapper.map_attributes(feature, new_feature_3, source_layer.fields(), object_type=object_type)

                    self._normalize_feature_rights(new_feature_3)
                    self._capitalize_all_fields(new_feature_3)

                    result_layer_3.addFeature(new_feature_3)
                    selected_count_3 += 1
            ...
            count_after_recheck_3 = selected_count_3
            ...
            if count_after_recheck_3 > 0:
                self._supplement_ez_field_from_extracts(result_layer_3)
    ...
    def perform_selection_oks(self, boundaries_exact: QgsVectorLayer,
                              source_layer: QgsVectorLayer,
                              progress_callback: Optional[Callable[[int, int], None]] = None) -> Optional[QgsVectorLayer]:
