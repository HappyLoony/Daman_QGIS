...
class ForestLoader:
    ...
    def check_forest_quarters(self):
        ...
        if self._check_existing_layer('L_1_7_3_ФГИС_ЛК_Кварталы'):
            log_info("Fsm_1_3_3: Слой L_1_7_3_ФГИС_ЛК_Кварталы найден")
            return True

        # Если не найден - НЕ загружаем!
        # F_1_3 Бюджет должен только анализировать, а не загружать
        log_warning("Fsm_1_3_3: Слой L_1_7_3_ФГИС_ЛК_Кварталы не найден. Загрузите слой через F_1_2 перед запуском бюджета.")
        return False
    ...
    def check_forest_subdivisions(self):
        ...
        """
        # Проверяем существование слоя
        if self._check_existing_layer('L_1_7_4_ФГИС_ЛК_Выделы'):
            log_info("Fsm_1_3_3: Слой L_1_7_4_ФГИС_ЛК_Выделы найден")
            return True

        # Если не найден - НЕ загружаем!
        log_warning("Fsm_1_3_3: Слой L_1_7_4_ФГИС_ЛК_Выделы не найден. Загрузите слой через F_1_2 перед запуском бюджета.")
        return False
