# -*- coding: utf-8 -*-
"""
Диалог классификации неопознанных объектов негативных процессов
...
class NegativClassifierDialog(BaseResponsiveDialog):
    ...
    def __init__(self, parent, feature, target_layers, current_index, total_count):
        """
        Инициализация диалога

        Args:
            parent: Родительское окно
            feature: QgsFeature объекта
            target_layers: Список доступных слоёв для выбора (list of dicts с full_name)
        ...
        self.setWindowTitle(f"Классификация негативных процессов ({current_index} из {total_count})")
        self.setModal(True)
    ...
    def init_ui(self):
        ...
        # Заголовок
        title_label = QLabel(
            f"Неопознанный негативный процесс ({self.current_index} из {self.total_count})"
        )
        ...
        instruction_label = QLabel(
            "Не удалось автоматически определить тип негативного процесса.\n"
        ...
        # Заполняем таблицу атрибутами из QgsFeature
        fields = self.feature.fields()
        self.attributes_table.setRowCount(fields.count())

        for row in range(fields.count()):
            field = fields.at(row)
            key = field.name()
            value = self.feature[key]
        ...
        # Группа выбора слоя
        selection_group = QGroupBox("Выбор слоя негативного процесса")
        ...
        # Фильтруем слои по префиксу L_4_1_
        negativ_only = [
            layer for layer in self.target_layers
            if layer.get('full_name', '').startswith('L_4_1_')
        ]

        # Сортируем по порядковому номеру в группе
        ...
        sorted_layers = sorted(negativ_only, key=get_sort_key)
        ...
        skip_btn.setToolTip("Пропустить этот объект, он попадёт в слой 'Негатив_Иные'")
        ...
        skip_all_btn.setToolTip("Пропустить все оставшиеся объекты, они попадут в слой 'Негатив_Иные'")
    ...
    def get_result(self):
        ...
        Returns:
            tuple: (selected_layer_full_name, skip_all_flag)
                   selected_layer_full_name - полное имя слоя или None (не копировать)
