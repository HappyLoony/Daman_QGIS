# -*- coding: utf-8 -*-
"""
Fsm_2_5_1_Dialog - Диалог выбора ЗУ для изъятия
...
- Кнопка "К изъятию" (копирование в L_2_3_1)
"""
...
class Fsm_2_5_1_Dialog(QDialog):
    ...
    def _setup_ui(self) -> None:
        ...
        description = QLabel(
            ...
            "и муниципальных нужд. Участки будут СКОПИРОВАНЫ в слой L_2_3_1_Изъятие_ЗУ."
        )
    ...
    def _load_features(self) -> None:
        ...
        self._update_count()
        log_info(f"Fsm_2_5_1: Загружено {len(features)} объектов из {self.current_layer.name()}")
    ...
    def _on_highlight(self) -> None:
        ...
        log_info(f"Fsm_2_5_1: Выделено {len(fids)} объектов на карте")
    ...
    def _on_row_double_clicked(self, row: int, column: int) -> None:
        ...
        if feature and feature.hasGeometry():
            ...
            iface.mapCanvas().setExtent(extent)
            iface.mapCanvas().refresh()
            log_info(f"Fsm_2_5_1: Переход к объекту fid={fid}")
