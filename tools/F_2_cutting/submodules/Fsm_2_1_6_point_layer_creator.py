# -*- coding: utf-8 -*-
"""
Fsm_2_1_6_PointLayerCreator - Создание точечных слоёв для нарезки ЗПР
...
class Fsm_2_1_6_PointLayerCreator:
    ...
    def create_point_layer(
        ...
        """
        if not points_data:
            log_warning(f"Fsm_2_1_6: Нет данных точек для слоя {layer_name}")
            return None
        ...
        try:
            ...
            if not features:
                log_warning(f"Fsm_2_1_6: Не удалось создать features для слоя {layer_name}")
                return None
            ...
            if not success:
                log_error(f"Fsm_2_1_6: Ошибка сохранения слоя {layer_name} в GPKG")
                return None
            ...
            if layer is not None and layer.isValid():
                log_info(f"Fsm_2_1_6: Создан точечный слой {layer_name} ({layer.featureCount()} точек)")
                return layer
            else:
                log_error(f"Fsm_2_1_6: Не удалось загрузить слой {layer_name} из GPKG")
                return None

        except Exception as e:
            log_error(f"Fsm_2_1_6: Ошибка создания слоя {layer_name}: {e}")
            return None
    ...
    def _save_to_gpkg(
        ...
        try:
            ...
            if not mem_layer.isValid():
                log_error(f"Fsm_2_1_6: Не удалось создать memory layer для {layer_name}")
                return False
            ...
            if error[0] != QgsVectorFileWriter.WriterError.NoError:
                log_error(f"Fsm_2_1_6: Ошибка записи в GPKG: {error[1]}")
                return False

            return True

        except Exception as e:
            log_error(f"Fsm_2_1_6: Исключение при сохранении в GPKG: {e}")
            return False
    ...
    def _load_layer_from_gpkg(self, layer_name: str) -> Optional[QgsVectorLayer]:
        ...
        if layer.isValid():
            ...
            log_error(f"Fsm_2_1_6: Не удалось загрузить слой из {uri}")
            return None
    ...
    def create_point_layer_for_polygon(
        ...
        if not point_layer_name:
            log_warning(f"Fsm_2_1_6: Нет соответствия для полигонального слоя {polygon_layer_name}")
            return None
