# -*- coding: utf-8 -*-
"""
Fsm_2_5_2_Transfer - Логика копирования ЗУ в слой Изъятие

Выполняет:
1. Создание слоя L_2_3_1_Изъятие_ЗУ (если не существует)
...
class Fsm_2_5_2_Transfer:
    ...
    def __init__(
        ...
        Args:
            ...
            target_layer_name: Имя целевого слоя (L_2_3_1_Изъятие_ЗУ)
    ...
    def execute(
        ...
        try:
            ...
            log_info(f"Fsm_2_5_2: Успешно скопировано {len(prepared_features)} объектов "
    ...
    def _get_or_create_target_layer(
        ...
        if existing_layers:
            ...
            if isinstance(layer, QgsVectorLayer) and layer.isValid():
                log_info(f"Fsm_2_5_2: Используем существующий слой {self.target_layer_name}")
                layer.startEditing()
                return layer
        ...
        if gpkg_layer.isValid():
            log_info(f"Fsm_2_5_2: Загружен слой {self.target_layer_name} из GPKG")
            gpkg_layer.startEditing()
            return gpkg_layer

        # Создать новый слой
        log_info(f"Fsm_2_5_2: Создание нового слоя {self.target_layer_name}")
    ...
    def _create_new_layer(
        ...
        try:
            ...
            if error[0] != QgsVectorFileWriter.WriterError.NoError:
                log_error(f"Fsm_2_5_2: Ошибка создания слоя в GPKG: {error[1]}")
                return None
            ...
            if new_layer.isValid():
                ...
                log_error(f"Fsm_2_5_2: Не удалось загрузить созданный слой")
                return None

        except Exception as e:
            log_error(f"Fsm_2_5_2: Ошибка создания слоя: {e}")
            return None
    ...
    def _prepare_features(
        ...
        for source_feat in features:
            ...
            if key in existing_keys:
                duplicates += 1
                log_warning(f"Fsm_2_5_2: Пропущен дубликат КН={kn}, Услов_КН={uslov_kn}")
                continue
        ...
        log_info(f"Fsm_2_5_2: Подготовлено {len(prepared)} features, дубликатов: {duplicates}")
    ...
    def _add_features_to_layer(
        ...
        try:
            ...
            if success:
                ...
                log_error("Fsm_2_5_2: Ошибка добавления features")
                return False
        except Exception as e:
            log_error(f"Fsm_2_5_2: Исключение при добавлении: {e}")
            return False
    ...
    def _renumber_ids(self, layer: QgsVectorLayer) -> None:
        ...
        if id_idx < 0:
            log_warning(f"Fsm_2_5_2: Поле ID не найдено в слое {layer.name()}")
            return
        ...
        log_info(f"Fsm_2_5_2: Перенумерованы ID в {layer.name()} ({len(features)} объектов)")
