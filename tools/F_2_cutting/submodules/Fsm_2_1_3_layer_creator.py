# -*- coding: utf-8 -*-
"""
Fsm_2_1_3_LayerCreator - Создание выходных слоёв для нарезки ЗПР
...
class Fsm_2_1_3_LayerCreator:
    ...
    def create_cutting_layer(
        ...
        Args:
            layer_name: Имя слоя (например, "Le_2_1_1_1_Раздел_ЗПР_ОКС")
        ...
        if not features_data:
            log_warning(f"Fsm_2_1_3: Нет данных для слоя {layer_name}")
            return None
        ...
        try:
            ...
            if not features:
                log_warning(f"Fsm_2_1_3: Не удалось создать features для слоя {layer_name}")
                return None
            ...
            if not success:
                log_error(f"Fsm_2_1_3: Ошибка сохранения слоя {layer_name} в GPKG")
                return None
            ...
            if layer and layer.isValid():
                log_info(f"Fsm_2_1_3: Создан слой {layer_name} ({layer.featureCount()} объектов)")
                return layer
            else:
                log_error(f"Fsm_2_1_3: Не удалось загрузить слой {layer_name} из GPKG")
                return None

        except Exception as e:
            log_error(f"Fsm_2_1_3: Ошибка создания слоя {layer_name}: {e}")
            return None
    ...
    def _save_to_gpkg(
        ...
        try:
            ...
            if not mem_layer.isValid():
                log_error(f"Fsm_2_1_3: Не удалось создать memory layer для {layer_name}")
                return False
            ...
            if error[0] != QgsVectorFileWriter.WriterError.NoError:
                log_error(f"Fsm_2_1_3: Ошибка записи в GPKG: {error[1]}")
                return False

            return True

        except Exception as e:
            log_error(f"Fsm_2_1_3: Исключение при сохранении в GPKG: {e}")
            return False
    ...
    def _load_layer_from_gpkg(self, layer_name: str) -> Optional[QgsVectorLayer]:
        ...
        if layer.isValid():
            ...
            log_error(f"Fsm_2_1_3: Не удалось загрузить слой из {uri}")
            return None
    ...
    def delete_existing_layer(self, layer_name: str) -> bool:
        ...
        try:
            ...
            if layer_idx >= 0:
                ...
                log_info(f"Fsm_2_1_3: Удалён существующий слой {layer_name}")

            ds = None
            return True

        except Exception as e:
            log_warning(f"Fsm_2_1_3: Не удалось удалить слой {layer_name}: {e}")
            return False
