# -*- coding: utf-8 -*-
"""
Fsm_2_1_5_KKMatcher - Привязка НГС к кадастровым кварталам
...
class Fsm_2_1_5_KKMatcher:
    ...
    def _build_index(self) -> None:
        ...
        for feature in self.kk_layer.getFeatures():
            ...
            if not self.is_valid_quarter(cad_num):
                log_warning(f"Fsm_2_1_5: КК '{cad_num}' отклонён (нулёвка или неверный формат)")
                continue

            log_info(f"Fsm_2_1_5: КК '{cad_num}' - валидный квартал")
        ...
        log_info(f"Fsm_2_1_5: Построен индекс КК: {valid_count} валидных кварталов из {total_count}")
    ...
    def match_ngs_to_quarters(
        ...
        if not self._spatial_index:
            log_warning("Fsm_2_1_5: Слой КК не инициализирован, привязка пропущена")
            return ngs_data
        ...
        log_info(f"Fsm_2_1_5: Привязка НГС к КК завершена: "
