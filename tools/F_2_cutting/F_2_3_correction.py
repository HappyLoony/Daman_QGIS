# -*- coding: utf-8 -*-
"""
F_2_3_Корректировка - Пересчёт атрибутов и точек после ручного редактирования нарезки
...
3. Пересчитывает атрибуты для всех слоёв:
   - ID (1, 2, 3...)
   - Услов_КН (глобальная нумерация по всем слоям)
...
class F_2_3_Correction(BaseTool):
    ...
    def get_name() -> str:
        ...
        return "F_2_3_Корректировка"

    def run(self) -> None:
        """Основной метод запуска инструмента (без диалога)"""
        log_info("F_2_3: Запуск корректировки")
    ...
    def _execute(self) -> None:
        ...
        log_info("F_2_3: Запуск корректировки нарезки")

        # 1. Инициализация субмодулей
        if not self.plugin_dir:
            log_error("F_2_3: Не установлен путь к плагину (plugin_dir)")
            return
        ...
        if not gpkg_path:
            log_error("F_2_3: Не найден путь к project.gpkg")
            return
        ...
        if not existing_layers:
            log_warning("F_2_3: Не найдено ни одного слоя нарезки")
            QMessageBox.warning(
                None,
                PLUGIN_NAME,
                "Не найдено слоёв нарезки.\n\n"
                "Сначала выполните нарезку через 'F_2_1_Нарезка ЗПР'."
            )
            return

        log_info(f"F_2_3: Найдено {len(existing_layers)} слоёв нарезки")
        ...
        log_info(f"F_2_3: Пересчёт НГС завершён, удалено объектов: {ngs_removed_count}")
        ...
        for poly_layer_name, points_layer_name, zpr_type in self.LAYER_ORDER:
            ...
            if poly_layer.featureCount() == 0:
                log_warning(f"F_2_3: Слой {poly_layer_name} пуст, удаляем")
        ...
        if self.layer_manager:
            ...
            log_info("F_2_3: Слои отсортированы по order_layers")

        # 8. Валидация минимальных площадей
        self._validate_min_areas()

        # 9. Финализация
        log_info(f"F_2_3: Корректировка завершена. "
                f"Обработано слоёв: {processed_layers}, "
                f"объектов: {total_features}, точек: {total_points}")

        if removed_layers:
            log_warning(f"F_2_3: Удалённые пустые слои: {', '.join(removed_layers)}")
    ...
    def _find_existing_layers(self) -> Dict[str, QgsVectorLayer]:
        ...
        for poly_layer_name, _, _ in self.LAYER_ORDER:
            ...
            if layers and isinstance(layers[0], QgsVectorLayer) and layers[0].isValid():
                ...
                log_info(f"F_2_3: Найден слой {poly_layer_name} "
    ...
    def _process_layer(
        ...
        if is_bez_mezh:
            ...
            # Только пересчёт ID, НЕ пересчитываем Услов_КН и План_*
            ...
            log_info(f"F_2_3: Слой Без_Меж {layer_name} обработан: "
        ...
        log_info(f"F_2_3: Слой {layer_name} обработан: "
    ...
    def _recalculate_bez_mezh_attributes(
        ...
        Услов_КН, План_*, Площадь_ОЗУ уже установлены в F_2_2
    ...
    def _collect_features_sorted_by_northwest(
        self,
        layer: QgsVectorLayer
    ) -> List[Dict[str, Any]]:
        """Сбор объектов из слоя, отсортированных по северо-западу

        Сортировка по СЗ обеспечивает правильный порядок контуров:
        - Контур ближайший к СЗ углу получает ID=1
        - Нумерация точек начинается с первого контура
    ...
    def _recalculate_attributes(
        ...
        - Услов_КН (глобальная нумерация, всегда)
    ...
    def _reassign_vri(
        ...
        if not self._vri_manager:
            log_warning("F_2_3: VRIAssignmentManager не инициализирован")
            return features_data

        # Получаем слой ЗПР по типу
        zpr_layer_name = self.ZPR_TYPE_TO_LAYER.get(zpr_type)
        if not zpr_layer_name:
            log_warning(f"F_2_3: Неизвестный тип ЗПР: {zpr_type}")
            return features_data

        project = QgsProject.instance()
        layers = project.mapLayersByName(zpr_layer_name)
        if not layers or not layers[0].isValid():
            log_warning(f"F_2_3: Слой ЗПР {zpr_layer_name} не найден, ВРИ не пересчитывается")
            return features_data

        zpr_layer = layers[0]
        log_info(f"F_2_3: Пересчёт ВРИ по слою {zpr_layer_name}")
    ...
    def _update_layer_in_gpkg(
        ...
        try:
            ...
            log_error(f"F_2_3: Ошибка обновления слоя {layer.name()}: {e}")
            if layer.isEditable():
                layer.rollBack()
            return False
    ...
    def _apply_styles_and_labels(
        ...
        log_info("F_2_3: Стили и подписи применены ко всем слоям")
    ...
    def _recalculate_all_ngs(
        ...
        for razdel_name, _, ngs_name, ngs_points_name, zpr_type in self.LAYER_PAIRS:
            ...
            if razdel_layer.featureCount() == 0:
                log_info(f"F_2_3: Слой Раздел {razdel_name} пуст, пропускаем пересчёт НГС")
                continue

            if ngs_layer.featureCount() == 0:
                log_info(f"F_2_3: Слой НГС {ngs_name} уже пуст")
                continue
    ...
    def _recalculate_ngs_for_pair(
        ...
        Returns:
            int: Количество удалённых объектов НГС
        """
        log_info(f"F_2_3: Пересчёт НГС {ngs_name} на основе Раздела ({zpr_type})")
        ...
        if not razdel_geometries:
            log_warning(f"F_2_3: Нет геометрий Раздела для {zpr_type}")
            return 0
        ...
        if razdel_union.isEmpty():
            log_warning(f"F_2_3: Объединение Разделов пустое для {zpr_type}")
            return 0

        log_info(f"F_2_3: Объединено {len(razdel_geometries)} объектов Раздел через unaryUnion")
        ...
        for feature in ngs_layer.getFeatures():
            ...
            if new_geom.isEmpty():
                ...
                features_to_delete.append(feature.id())
                log_info(f"F_2_3: Объект НГС ID={feature.id()} полностью покрыт Разделами, удаляем")
            else:
        ...
        if features_to_delete or features_to_update:
            ...
            if features_to_delete:
                ...
                log_info(f"F_2_3: Удалено {len(features_to_delete)} объектов НГС из {ngs_name}")
    ...
    def _validate_min_areas(self) -> None:
        ...
        try:
            ...
            log_error(f"F_2_3: Ошибка валидации минимальных площадей: {e}")
    ...
    def _remove_empty_layer(
        ...
        log_info(f"F_2_3: Слой {poly_layer_name} удалён из проекта")
        ...
        if points_layer_name:
            ...
            for points_layer in points_layers:
                ...
                log_info(f"F_2_3: Точечный слой {points_layer_name} удалён из проекта")
            ...
            if ds:
                ...
                for i in range(ds.GetLayerCount()):
                    ...
                    if lyr and lyr.GetName() == poly_layer_name:
                        ds.DeleteLayer(i)
                        log_info(f"F_2_3: Слой {poly_layer_name} удалён из GPKG")
                        break
                ...
                if points_layer_name:
                    for i in range(ds.GetLayerCount()):
                        ...
                        if lyr and lyr.GetName() == points_layer_name:
                            ds.DeleteLayer(i)
                            log_info(f"F_2_3: Слой {points_layer_name} удалён из GPKG")
                            break

                ds = None  # Закрываем соединение
        except Exception as e:
            log_warning(f"F_2_3: Не удалось удалить слои из GPKG: {e}")
