# -*- coding: utf-8 -*-
"""
F_2_5_Изъятие - Ручной отбор ЗУ для изъятия под гос./муниц. нужды

Позволяет вручную выбрать земельные участки из слоёв нарезки (Раздел, НГС)
и скопировать их в слой L_2_3_1_Изъятие_ЗУ.
...
Последовательность: F_2_1 -> F_2_2 -> F_2_5 -> F_2_3 -> F_2_4
"""
...
# Импорт субмодулей
from .submodules.Fsm_2_5_1_dialog import Fsm_2_5_1_Dialog
from .submodules.Fsm_2_5_2_transfer import Fsm_2_5_2_Transfer

if TYPE_CHECKING:
    from Daman_QGIS.managers import LayerManager


class F_2_5_Izyatie(BaseTool):
    ...
    def __init__(self, iface: Any) -> None:
        ...
        self._transfer: Optional[Fsm_2_5_2_Transfer] = None
        self._dialog: Optional[Fsm_2_5_1_Dialog] = None
    ...
    def get_name() -> str:
        ...
        return "F_2_5_Изъятие"

    def run(self) -> None:
        """Основной метод запуска инструмента"""
        log_info("F_2_5: Запуск отбора ЗУ для изъятия")
        ...
        if not available_layers:
            self.iface.messageBar().pushMessage(
                PLUGIN_NAME,
                "Не найдены слои нарезки. Сначала выполните нарезку (F_2_1).",
                level=Qgis.Warning,
                duration=MESSAGE_INFO_DURATION
            )
            log_warning("F_2_5: Не найдены слои нарезки для отбора")
            return
        ...
        if not gpkg_path:
            log_error("F_2_5: Не удалось получить путь к GPKG")
            return

        if not self.plugin_dir:
            log_error("F_2_5: Не установлен plugin_dir")
            return

        self._transfer = Fsm_2_5_2_Transfer(
            gpkg_path=gpkg_path,
        ...
        for layer_name in self.SOURCE_LAYERS:
            ...
            if layers:
                ...
                if isinstance(layer, QgsVectorLayer) and layer.featureCount() > 0:
                    ...
                    log_info(f"F_2_5: Найден слой {layer_name} "
    ...
    def _on_transfer(
        ...
        if not self._transfer:
            log_error("F_2_5: Модуль переноса не инициализирован")
        ...
        if result.get('error'):
            log_error(f"F_2_5: Ошибка копирования: {result['error']}")
