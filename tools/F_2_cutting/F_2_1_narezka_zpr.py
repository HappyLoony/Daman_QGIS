# -*- coding: utf-8 -*-
"""
F_2_1_Нарезка ЗПР - Нарезка зон планируемого размещения

Нарезка ВСЕХ типов ЗПР (стандартные и рекреационные) по границам
земельных участков и других слоёв (НП, МО, Лес, Вода).

Типы ЗПР:
- Стандартные: ОКС, ПО (площадные объекты), ВО -> Le_2_1_*
- Рекреационные: РЕК_АД, СЕТИ_ПО, СЕТИ_ВО, НЭ -> Le_2_2_*

Создаёт слои:
- Le_2_X_Y_1_Раздел_* - части ЗПР внутри границ ЗУ
- Le_2_X_Y_2_НГС_* - части ЗПР вне границ ЗУ (незанятые земли)

Примечание:
    Логика нарезки делегируется в M_26_CuttingManager.
    F_2_1 является единой UI-точкой входа для всех типов нарезки.
"""
...
from Daman_QGIS.constants import (
    ...
    # Слои границ (overlay)
    LAYER_SELECTION_NP, LAYER_SELECTION_MO, LAYER_EGRN_LES, LAYER_SELECTION_VODA,
    # Выходные слои
...
from Daman_QGIS.managers.infrastructure import track_exception

if TYPE_CHECKING:
    from Daman_QGIS.managers import LayerManager


class F_2_1_NarezkaZPR(BaseTool):
    ...
    def run(self) -> None:
        ...
        """
        log_info("F_2_1: Запуск нарезки ЗПР")
    ...
    def _execute_via_m26(self) -> None:
        ...
        try:
            ...
            cutting_manager = registry.get('M_26')
            cutting_manager.set_layer_manager(self.layer_manager)
            ...
            # Строгая проверка VRI: если есть пустые/невалидные VRI - блокируем нарезку
            vri_errors_std = avail_std.get('vri_errors', [])
            vri_errors_rek = avail_rek.get('vri_errors', [])
            all_vri_errors = vri_errors_std + vri_errors_rek

            if all_vri_errors:
                error_msg = "Слои ЗПР содержат пустые или невалидные значения VRI:\n\n"
                for vri_err in all_vri_errors:
                    error_msg += f"Слой {vri_err['layer']}:\n"
                    for err in vri_err['errors']:
                        error_msg += f"  - {err}\n"
                error_msg += "\nЗаполните поле VRI в атрибутах слоя ЗПР перед нарезкой."
                self._show_error(error_msg)
                return
            ...
            if not avail_std['can_cut'] and not avail_rek['can_cut']:
                self._show_error(
                    ...
                    "- Есть хотя бы один слой ЗПР с заполненным полем VRI"
                )
                return

            total_zpr = len(avail_std.get('available_zpr', [])) + len(avail_rek.get('available_zpr', []))
            log_info(f"F_2_1: Доступно {total_zpr} типов ЗПР для нарезки")
            ...
            log_error(f"F_2_1: Ошибка при нарезке: {e}")
            import traceback
            log_error(traceback.format_exc())
            track_exception("F_2_1", e)
