# -*- coding: utf-8 -*-
"""
Fsm_3_2_1_Dialog - Диалог выбора ЗУ для переноса в Без_Меж
...
class Fsm_3_2_1_Dialog(QDialog):
    ...
    def _setup_ui(self) -> None:
        ...
        description = QLabel(
            ...
            "Эти ЗУ будут перенесены из слоя Раздел в слой Без_Меж."
        )
    ...
    def _load_features(self) -> None:
        ...
        self._update_count()
        log_info(f"Fsm_3_2_1: Загружено {len(features)} объектов из {self.current_layer.name()}")
    ...
    def _on_highlight(self) -> None:
        ...
        log_info(f"Fsm_3_2_1: Выделено {len(fids)} объектов на карте")
    ...
    def _on_row_double_clicked(self, row: int, column: int) -> None:
        ...
        if feature and feature.hasGeometry():
            ...
            iface.mapCanvas().setExtent(extent)
            iface.mapCanvas().refresh()
            log_info(f"Fsm_3_2_1: Переход к объекту fid={fid}")
    ...
    def _on_transfer(self) -> None:
        ...
        reply = QMessageBox.question(
            ...
            f"Перенести {len(fids)} объект(ов) в слой Без_Меж?\n\n"
