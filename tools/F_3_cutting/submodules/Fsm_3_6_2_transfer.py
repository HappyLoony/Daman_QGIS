# -*- coding: utf-8 -*-
"""
Fsm_3_6_2_Transfer - Логика переноса ЗУ из Раздел в Изм (Изменяемые)

Выполняет:
1. Создание слоя Изм (если не существует)
2. Копирование features из Раздел в Изм
...
class Fsm_3_6_2_Transfer:
    """Модуль переноса features из Раздел в Изм"""
    ...
    def __init__(
        ...
        Args:
            ...
            razdel_to_izm: Маппинг имён слоёв Раздел -> Изм
            work_type: Значение поля Вид_Работ для Изм
            layer_manager: Менеджер слоёв (опционально)
        """
        self.gpkg_path = gpkg_path
        self.razdel_to_izm = razdel_to_izm
    ...
    def execute(
        ...
        """
        log_info(f"Fsm_3_6_2: Перенос {len(feature_ids)} объектов из {source_layer.name()}")

        # Определить целевой слой
        source_name = source_layer.name()
        target_name = self.razdel_to_izm.get(source_name)

        if not target_name:
            # Попробуем по имени без констант
            for razdel, izm in self.razdel_to_izm.items():
        ...
        try:
            ...
            # 3. Подготовить атрибуты для Изм
            prepared_features = self._prepare_izm_features(
            ...
            log_info(f"Fsm_3_6_2: Успешно перенесено {len(feature_ids)} объектов в {target_name}")
            ...
            log_error(f"Fsm_3_6_2: Исключение при переносе: {e}")
            return {'error': str(e)}
    ...
    def _get_or_create_target_layer(
        ...
        if existing_layers:
            ...
            if isinstance(layer, QgsVectorLayer) and layer.isValid():
                log_info(f"Fsm_3_6_2: Используем существующий слой {target_name}")
                layer.startEditing()
                return layer
        ...
        if gpkg_layer.isValid():
            log_info(f"Fsm_3_6_2: Загружен слой {target_name} из GPKG")
            gpkg_layer.startEditing()
            return gpkg_layer

        # Создать новый слой
        log_info(f"Fsm_3_6_2: Создание нового слоя {target_name}")
    ...
    def _create_new_layer(
        ...
        try:
            ...
            if not mem_layer.isValid():
                log_error(f"Fsm_3_6_2: Не удалось создать memory layer")
                return None
            ...
            if error[0] != QgsVectorFileWriter.WriterError.NoError:
                log_error(f"Fsm_3_6_2: Ошибка создания слоя в GPKG: {error[1]}")
                return None
            ...
            if new_layer.isValid():
                ...
                log_error(f"Fsm_3_6_2: Не удалось загрузить созданный слой")
                return None

        except Exception as e:
            log_error(f"Fsm_3_6_2: Ошибка создания слоя: {e}")
            return None

    def _prepare_izm_features(
        self,
        features: List[QgsFeature],
        fields: QgsFields
    ) -> List[QgsFeature]:
        """Подготовить features с атрибутами для Изм
        ...
        for source_feat in features:
            ...
            # Перезаписываем специфичные для Изм
        ...
        log_info(f"Fsm_3_6_2: Подготовлено {len(prepared)} features для Изм")
        return prepared
    ...
    def _add_features_to_layer(
        ...
        try:
            ...
            if success:
                ...
                log_error("Fsm_3_6_2: Ошибка добавления features")
                return False
        except Exception as e:
            log_error(f"Fsm_3_6_2: Исключение при добавлении: {e}")
            return False
    ...
    def _delete_features_from_layer(
        ...
        try:
            ...
            if success:
                ...
                log_error("Fsm_3_6_2: Ошибка удаления features")
                return False
        except Exception as e:
            log_error(f"Fsm_3_6_2: Исключение при удалении: {e}")
            return False
    ...
    def _renumber_ids(self, layer: QgsVectorLayer) -> None:
        ...
        if id_idx < 0:
            log_warning(f"Fsm_3_6_2: Поле ID не найдено в слое {layer.name()}")
            return
        ...
        log_info(f"Fsm_3_6_2: Перенумерованы ID в {layer.name()} ({len(features)} объектов)")
    ...
    def _add_layer_to_project(
        ...
        log_info(f"Fsm_3_6_2: Слой {layer_name} добавлен в проект")
        ...
        if self.layer_manager:
            try:
                ...
                log_warning(f"Fsm_3_6_2: Не удалось применить стили: {e}")
