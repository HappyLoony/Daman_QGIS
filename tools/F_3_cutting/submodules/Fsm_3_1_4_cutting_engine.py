# -*- coding: utf-8 -*-
"""
Fsm_3_1_4_CuttingEngine - Движок нарезки ЗПР
...
from .Fsm_3_1_5_kk_matcher import Fsm_3_1_5_KKMatcher
from .Fsm_3_1_6_point_layer_creator import Fsm_3_1_6_PointLayerCreator

# Минимальная площадь НГС (м²)
...
# населённых пунктов = 0.10 м. Площадь 0.10 м² = квадрат ~31.6x31.6 см.
MIN_NGS_AREA = 0.10  # м²


class Fsm_3_1_4_CuttingEngine:
    """Движок выполнения нарезки ЗПР"""

    def __init__(
        self,
        geometry_processor: 'Fsm_3_1_1_GeometryProcessor',
        attribute_mapper: 'Fsm_3_1_2_AttributeMapper',
        layer_creator: 'Fsm_3_1_3_LayerCreator'
    ) -> None:
        ...
        self.point_layer_creator = Fsm_3_1_6_PointLayerCreator(layer_creator.gpkg_path)
    ...
    def _get_overlay_union(
        ...
        if overlay_type not in self._overlay_union_cache:
            ...
            log_info(f"Fsm_3_1_4: Создан кэш union для {overlay_type}")
    ...
    def process_zpr_type(
        ...
        log_info(f"Fsm_3_1_4: Начало нарезки {zpr_type}")

        # Сброс статистики и счётчиков ID (для слоя)
        ...
        self.attribute_mapper.reset_id_counter(razdel_layer_name)
        self.attribute_mapper.reset_id_counter(ngs_layer_name)
        ...
        try:
            ...
            log_info(f"Fsm_3_1_4: Первичная нарезка: {len(razdel_data)} Раздел, "
                    f"{len(ngs_data)} НГС")
            ...
            razdel_features = self._generate_features_data(
                razdel_data, razdel_layer_name, zpr_type
            )
            ngs_features = self._generate_features_data(
                ngs_data, ngs_layer_name, zpr_type
            )
            ...
            # ВАЖНО: Раздел и НГС из одной ЗПР должны иметь ОБЩУЮ сквозную нумерацию,
            # так как на графике они отображаются вместе. Создаём один manager
            # и передаём его в оба вызова (для НГС без сброса счётчика).
            razdel_points_data = []
            ngs_points_data = []
            point_numbering_manager = PointNumberingManager()

            if razdel_features:
                razdel_features, razdel_points_data = self._number_points(
                    razdel_features, razdel_layer_name, point_numbering_manager
                )

            if ngs_features:
                # Передаём тот же manager - нумерация продолжится с текущего значения
                ngs_features, ngs_points_data = self._number_points(
                    ngs_features, ngs_layer_name, point_numbering_manager
                )
            ...
            log_info(f"Fsm_3_1_4: TODO - Без_Меж и ПС для {zpr_type}")
            ...
            log_info(f"Fsm_3_1_4: {zpr_type} завершено за "
            ...
            self.statistics['processing_time'] = time.time() - start_time
            log_error(f"Fsm_3_1_4: Ошибка нарезки {zpr_type}: {e}")
            return None
        ...
        for zpr_feature in zpr_layer.getFeatures():
            ...
            if not zu_union.isEmpty():
                ...
                if not ngs_geom.isEmpty():
                    ...
                    log_info(f"Fsm_3_1_4: НГС от ЗПР ID={zpr_id}: общая площадь={ngs_total_area:.4f} м2, "
                            f"isMultipart={ngs_geom.isMultipart()}")

                    # Извлекаем отдельные полигоны
                    extracted_polygons = self.geometry_processor.extract_polygons(ngs_geom)
                    log_info(f"Fsm_3_1_4: НГС от ЗПР ID={zpr_id}: извлечено {len(extracted_polygons)} полигонов")

                    filtered_count = 0
                    for idx, poly_geom in enumerate(extracted_polygons):
                        if poly_geom.isEmpty():
                            log_warning(f"Fsm_3_1_4: НГС #{idx} от ЗПР ID={zpr_id}: пустая геометрия, пропуск")
                            continue
                        ...
                        if poly_area < MIN_NGS_AREA:
                            filtered_count += 1
                            log_info(f"Fsm_3_1_4: НГС #{idx} от ЗПР ID={zpr_id}: "
                                    f"площадь={poly_area:.6f} м2 < {MIN_NGS_AREA} м2, пропуск (артефакт)")
                            continue

                        log_info(f"Fsm_3_1_4: НГС #{idx} от ЗПР ID={zpr_id}: площадь={poly_area:.4f} м2")
                    ...
                    if filtered_count > 0:
                        log_info(f"Fsm_3_1_4: НГС от ЗПР ID={zpr_id}: отфильтровано {filtered_count} "
    ...
    def _cut_by_overlays(
        ...
        for overlay_type, overlay_layer in overlay_layers.items():
            ...
            log_info(f"Fsm_3_1_4: Дополнительная нарезка по {overlay_type}")
            ...
            result = new_result
            log_info(f"Fsm_3_1_4: После нарезки по {overlay_type}: {len(result)} объектов")
    ...
    def _generate_features_data(
        ...
        for item in data:
            ...
            attributes = self.attribute_mapper.fill_generated_fields(
                zu_attrs, geom, layer_name, zpr_type
            )
    ...
    def _build_spatial_index(
        ...
        log_info(f"Fsm_3_1_4: Построен пространственный индекс ({len(features_dict)} объектов)")
    ...
    def _number_points(
        ...
        layer_name: str,
        manager: Optional[PointNumberingManager] = None
        ...
        Args:
            ...
            manager: Внешний менеджер нумерации для сквозной нумерации.
                    Если None - создаётся новый. Если передан - нумерация
                    продолжается с текущего значения (auto_reset=False).
        ...
        # Если manager передан извне - используем его БЕЗ сброса (auto_reset=False)
        # Если manager=None - создаём новый с auto_reset=True (по умолчанию)
        if manager is None:
        ...
        processed_data, points_data = manager.process_polygon_layer(numbering_input, auto_reset=auto_reset)
        ...
        log_info(f"Fsm_3_1_4: Нумерация точек для {layer_name}: "
    ...
    def _create_bez_mezh(self) -> None:
        ...
        """
        log_info("Fsm_3_1_4: TODO - _create_bez_mezh не реализовано")
    ...
    def _create_ps(self) -> None:
        ...
        TODO: Реализовать позже
        """
        log_info("Fsm_3_1_4: TODO - _create_ps не реализовано")
