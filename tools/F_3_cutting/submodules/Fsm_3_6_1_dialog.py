# -*- coding: utf-8 -*-
"""
Fsm_3_6_1_Dialog - Диалог выбора ЗУ для переноса в Изм (Изменяемые)

GUI диалог с таблицей для выбора земельных участков,
которые сохраняют границы и КН, но меняют атрибуты.
...
class Fsm_3_6_1_Dialog(QDialog):
    """Диалог выбора ЗУ для переноса в слой Изм (Изменяемые)

    Attributes:
        layers: Список доступных слоёв Раздел
        transfer_callback: Функция переноса (source_layer, feature_ids) -> result
    """
    ...
    def __init__(
        ...
        Args:
            ...
            layers: Список доступных слоёв Раздел
            transfer_callback: Callback функция для выполнения переноса
        """
        ...
        self.setWindowTitle("Перенос ЗУ в слой Изменяемые")
        self.resize(800, 500)
        self.setModal(True)
    ...
    def _setup_ui(self) -> None:
        ...
        # Заголовок
        title = QLabel("<h3>Перенос ЗУ в слой \"Изменяемые\"</h3>")
        title.setAlignment(Qt.AlignmentFlag.AlignCenter)
        layout.addWidget(title)

        # Описание
        description = QLabel(
            "Выберите земельные участки, которые сохраняют границы и кадастровый номер,\n"
            "но будут менять атрибуты (ВРИ, категория и т.д.).\n"
            "Эти ЗУ будут перенесены из слоя Раздел в слой Изм."
        )
    ...
    def _load_features(self) -> None:
        ...
        self._update_count()
        log_info(f"Fsm_3_6_1: Загружено {len(features)} объектов из {self.current_layer.name()}")
    ...
    def _on_highlight(self) -> None:
        ...
        self._rubber_band.setColor(QColor(255, 165, 0, 100))  # Оранжевый для Изм
        ...
        log_info(f"Fsm_3_6_1: Выделено {len(fids)} объектов на карте")
    ...
    def _on_row_double_clicked(self, row: int, column: int) -> None:
        ...
        if feature and feature.hasGeometry():
            ...
            iface.mapCanvas().setExtent(extent)
            iface.mapCanvas().refresh()
            log_info(f"Fsm_3_6_1: Переход к объекту fid={fid}")
    ...
    def _on_transfer(self) -> None:
        ...
        reply = QMessageBox.question(
            ...
            f"Перенести {len(fids)} объект(ов) в слой Изм?\n\n"
            "Объекты будут удалены из слоя Раздел.\n"
            "Атрибуты (ВРИ, категория) можно будет изменить позже.",
