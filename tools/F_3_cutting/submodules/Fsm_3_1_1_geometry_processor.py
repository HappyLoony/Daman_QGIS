# -*- coding: utf-8 -*-
"""
Fsm_3_1_1_GeometryProcessor - Геометрические операции для нарезки ЗПР
...
# Минимальная площадь полигона (м²)
# Полигоны меньше этого порога считаются артефактами и удаляются при округлении.
# Синхронизировано с MIN_NGS_AREA в Fsm_3_1_4_cutting_engine.py
MIN_VALID_AREA = 0.10  # м² (квадрат ~31.6x31.6 см)


class Fsm_3_1_1_GeometryProcessor:
    ...
    def create_union(self, layer: QgsVectorLayer) -> QgsGeometry:
        ...
        log_info(f"Fsm_3_1_1: Создан union из {len(geometries)} геометрий")
        return result
    ...
    def cut_by_boundaries(
        ...
        log_info(f"Fsm_3_1_1: Нарезка по {boundary_field}: "
    ...
    def _snap_to_grid(self, geom: QgsGeometry) -> QgsGeometry:
        ...
        if result.isEmpty() and area_before > 0:
            ...
            log_warning(f"Fsm_3_1_1: snap схлопнул значимую геометрию ({area_before:.2f} м2)")
            return geom
        ...
        if not result.isEmpty() and geom.type() == 2 and result.type() != 2:
            if area_before < MIN_VALID_AREA:
                return QgsGeometry()  # Артефакт - удаляем
            log_warning(f"Fsm_3_1_1: snap деградировал полигон ({area_before:.2f} м2)")
            return geom
        ...
        if not result.isGeosValid():
            ...
            if geom.type() == 2 and type_after != 2:
                if area_before < MIN_VALID_AREA:
                    return QgsGeometry()  # Артефакт - удаляем
                log_warning(f"Fsm_3_1_1: makeValid деградировал полигон ({area_before:.2f} м2)")
                return geom
    ...
    def validate_and_fix(self, geom: QgsGeometry) -> QgsGeometry:
        ...
        if not geom.isGeosValid():
            ...
            if fixed.isEmpty():
                log_warning("Fsm_3_1_1: Не удалось исправить невалидную геометрию")
                return QgsGeometry()
            return fixed
