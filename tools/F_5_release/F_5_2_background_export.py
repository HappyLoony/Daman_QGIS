...
POZU_DEFAULT_LAYERS = ['Le_2_1_', 'Le_2_2_', 'Le_2_5_', 'Le_2_6_']


class F_5_2_BackgroundExport(BaseTool):
    ...
    def run(self) -> None:
        ...
        log_info("F_5_2: Запуск экспорта подложек")
        ...
        log_info(f"F_5_2: Папка для сохранения подложек: {output_folder}")
    ...
    def _get_output_folder(self) -> Optional[str]:
        ...
        try:
            ...
            log_error("F_5_2: M_19 не активен, невозможно определить папку")
            return None

        except Exception as e:
            log_error(f"F_5_2: Ошибка определения папки для сохранения: {str(e)}")
            return None
    ...
    def _load_background_templates(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            log_info(f"F_5_2: Загружено {len(templates)} шаблонов подложек")
            return templates

        except Exception as e:
            log_error(f"F_5_2: Ошибка загрузки шаблонов подложек: {str(e)}")
            return []
    ...
    def _export_backgrounds(self,
        ...
        """
        log_info(f"F_5_2: Экспорт {len(templates)} подложек в {output_folder}")
        ...
        for template in templates:
            if progress.wasCanceled():
                log_warning("F_5_2: Экспорт отменен пользователем")
                break
    ...
    def _export_single_background(self,
        ...
        try:
            ...
            if not layer_names:
                log_warning(f"F_5_2: Шаблон {name} не содержит слоёв, пропускаем")
                return False, {}
            ...
            if not file_name:
                log_warning(f"F_5_2: Шаблон {name} не содержит допустимого имени файла")
                return False, {}
            ...
            for layer_pattern in layer_names:
                # Защита от None значений в списке слоёв
                ...
                if found_layers:
                    ...
                    if layer_pattern.endswith('_'):
                        log_info(f"F_5_2: По префиксу '{layer_pattern}' найдено {len(found_layers)} слоёв")
                else:
                    log_warning(f"F_5_2: Слой/префикс '{layer_pattern}' не найден в проекте")

            if not qgis_layers:
                log_warning(f"F_5_2: Ни один слой из шаблона {name} не найден в проекте")
                return False, {}
            ...
            log_info(f"F_5_2: Экспорт подложки в {output_path}")
            ...
            if success:
                log_success(f"F_5_2: Подложка {os.path.basename(output_path)} экспортирована успешно")
            else:
                log_error(f"F_5_2: Ошибка экспорта подложки {os.path.basename(output_path)}")

            return success, {name: success}

        except Exception as e:
            log_error(f"F_5_2: Ошибка экспорта подложки {template.get('name', 'Unknown')}: {str(e)}")
            return False, {}
    ...
    def _export_pozu_background(self, output_folder: str, ref_managers) -> Tuple[bool, Dict[str, bool]]:
        ...
        if self._has_stage_layers():
            log_info("F_5_2: П_ОЗУ - обнаружена этапность, экспорт в 3 файла")
            results = self._export_pozu_with_stages(output_folder, ref_managers)
        else:
            log_info("F_5_2: П_ОЗУ - этапность не обнаружена, экспорт в 1 файл")
    ...
    def _show_results(self, results: Dict[str, bool], output_folder: str) -> None:
        ...
        log_info(f"F_5_2: Экспорт завершен: {success_count} успешно, {error_count} ошибок")

    def _has_stage_layers(self) -> bool:
        """
        Проверить наличие слоёв этапности в проекте

        Этапность определяется по наличию слоёв с префиксами Le_2_7_ или Le_2_8_
        ...
        for stage_info in POZU_STAGE_LAYERS.values():
            for prefix in [stage_info['polygons'], stage_info['points']]:
                # Пропускаем None (например, точечный слой для итога)
                if not prefix:
                    continue
                found_layers = layer_finder.find_layers_by_pattern(prefix, QgsVectorLayer)
                for layer in found_layers:
                    if layer.featureCount() > 0:
                        log_info(f"F_5_2: Найден слой этапности {layer.name()} с {layer.featureCount()} объектами")
                        return True

        log_info("F_5_2: Слои этапности не найдены или пусты")
        return False
    ...
    def _get_temporary_zu_ids(self, stage2_layers: List[QgsVectorLayer]) -> Set[int]:
        ...
        Args:
            stage2_layers: Слои 2 этапа (полигоны Le_2_7_2_*)
        ...
        if temporary_ids:
            log_info(f"F_5_2: Найдено {len(temporary_ids)} временных ЗУ для исключения из итога: {sorted(temporary_ids)}")
    ...
    def _create_filtered_memory_layer(
        ...
        if id_idx < 0:
            log_warning(f"F_5_2: Поле '{id_field}' не найдено в слое {source_layer.name()}")
            return source_layer
        ...
        if not memory_layer or not memory_layer.isValid():
            log_error(f"F_5_2: Не удалось создать memory-слой для {source_layer.name()}")
            return source_layer
        ...
        log_info(f"F_5_2: Фильтрация слоя {source_layer.name()}: "
    ...
    def _export_pozu_with_stages(self, output_folder: str, ref_managers) -> Dict[str, bool]:
        ...
        for stage_name, prefixes in POZU_STAGE_LAYERS.items():
            ...
            if not qgis_layers:
                log_warning(f"F_5_2: Не найдены слои для {filename}")
                results[filename] = False
                continue

            log_info(f"F_5_2: Экспорт {filename} ({len(qgis_layers)} слоёв) в {output_path}")
            ...
            try:
                ...
                if success:
                    log_success(f"F_5_2: {filename} экспортирован успешно")
                else:
                    log_error(f"F_5_2: Ошибка экспорта {filename}")
            except Exception as e:
                log_error(f"F_5_2: Ошибка экспорта {filename}: {str(e)}")
                results[filename] = False
    ...
    def _export_pozu_without_stages(self, output_folder: str, ref_managers) -> Dict[str, bool]:
        ...
        for prefix in POZU_DEFAULT_LAYERS:
            ...
            qgis_layers.extend(found_layers)
            if found_layers:
                log_info(f"F_5_2: По префиксу '{prefix}' найдено {len(found_layers)} слоёв")

        if not qgis_layers:
            log_warning(f"F_5_2: Не найдены слои нарезки для {filename}")
