# -*- coding: utf-8 -*-
"""
Fsm_5_3_5 - Утилиты экспорта документов
...
from qgis.core import (
    Qgis, QgsVectorLayer, QgsGeometry, QgsProject,
    QgsCoordinateReferenceSystem, QgsCoordinateTransform, QgsPointXY
)

from Daman_QGIS.managers import CoordinatePrecisionManager as CPM, registry
...
class ExportUtils:
    ...
    def extract_polygon_contours(
        ...
        """
        if not geometry or geometry.type() != Qgis.GeometryType.Polygon:
            return []
    ...
    def ensure_folder_exists(folder_path: str) -> bool:
        ...
        try:
            ...
            log_warning(f"Fsm_5_3_5: Ошибка создания папки {folder_path}: {str(e)}")
            return False
    ...
    def format_template_text(
        ...
        """
        Форматировать текст шаблона с подстановкой переменных

        Переменные в тексте в фигурных скобках: {variable_name}
        ...
        Args:
            ...
            metadata: Метаданные проекта из GeoPackage
            layer_info: Дополнительная информация (layer_name, area, hole_number, ...)
