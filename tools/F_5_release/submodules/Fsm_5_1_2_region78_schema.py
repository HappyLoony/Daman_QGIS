...
# === Правила автоматического расчёта площадей ===
#
# Площадь вычисляется из геометрии объекта.
# Ключ — имя слоя DPT_*, значение — dict: имя_поля -> {unit, decimals}
#
# unit:
#   'sqm' — квадратные метры (значение = area_m2, округляется до целого)
#   'ha'  — гектары (значение = area_m2 / 10000, округляется до decimals знаков)
#
# decimals:
#   0 — целое число (для кв.м: round() → int as float или int)
#   2 — п.4.3: площадь элемента планировочной структуры
#   4 — п.4.8: площадь зелёных насаждений
#
# Источник: Приказ КГА N 1-16-82 от 04.07.2025, пп. 4.1-4.9

AREA_RULES: Dict[str, Dict[str, Dict[str, Any]]] = {
    # п.4.1 — Площадь застройки (кв.м, Вещественное → целое значение)
    'DPT_OKS_PL': {
        'Площадь_застр': {'unit': 'sqm', 'decimals': 0},
    },
    # п.4.5 — Площадь ЗУ (кв.м, Целое)
    'DPT_O_ZU': {
        'Площадь_ЗУ': {'unit': 'sqm', 'decimals': 0},
    },
    'DPT_I_ZU': {
        'Площадь_ЗУ': {'unit': 'sqm', 'decimals': 0},
    },
    ...
    # п.4.3 — Площадь элемента планировочной структуры (га, 2 знака)
    'DPT_P_PS': {
        'Площадь_га': {'unit': 'ha', 'decimals': 2},
    },
    'DPT_S_PS': {
        'Площадь_га': {'unit': 'ha', 'decimals': 2},
    },
    # п.4.8 — Площадь зелёных насаждений (га, 4 знака)
    'DPT_ZEL_ZNOP': {
        'Площадь_га': {'unit': 'ha', 'decimals': 4},
    },
    'DPT_ZEL_ZU': {
        'Площадь_га': {'unit': 'ha', 'decimals': 4},
    },
    # п.4.9 — Площадь парковки (кв.м, Вещественное → целое значение)
    'DPT_PARK': {
        'Площадь': {'unit': 'sqm', 'decimals': 0},
    },
    'DPT_PARK_ZU': {
        'Площадь': {'unit': 'sqm', 'decimals': 0},
    },
