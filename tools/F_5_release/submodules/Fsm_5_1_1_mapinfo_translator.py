...
class Fsm_5_1_1_MapInfoTranslator:
    ...
    def parse_mapinfo_style(self, style_string: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            log_warning(f"Fsm_5_1_1: Ошибка парсинга MapInfo стиля '{style_string}': {str(e)}")
            return None
    ...
    def get_style_for_layer(self, layer_name: str) -> Optional[str]:
        ...
        if not layer_info:
            log_warning(f"Fsm_5_1_1: Слой {layer_name} не найден в Base_layers.json")
            return None
        ...
        if style and style != '-':
            log_info(f"Fsm_5_1_1: Получен стиль MapInfo для слоя {layer_name}: {style}")
            return style

        log_warning(f"Fsm_5_1_1: Стиль MapInfo не задан для слоя {layer_name}")
        return None
    ...
    def get_all_layers_with_styles(self) -> Dict[str, str]:
        ...
        log_info(f"Fsm_5_1_1: Найдено {len(result)} слоев с MapInfo стилями")
        return result
