...
class Fsm_5_3_1_CoordinateList:
    ...
    def _collect_points_from_point_layer(
        ...
        ВНИМАНИЕ (заметка 2026-07-09, code-smell): метод вызывается ТОЛЬКО когда на
        вход _collect_contours_with_coordinates подан ТОЧЕЧНЫЙ слой (ветка Point).
        Текущие продукты (перечни ОЗУ/ПС/ГПМТ) подают ПОЛИГОНАЛЬНЫЕ слои — тип кольца
        там берётся из геометрии полигона, этот метод НЕ вызывается (мёртвый путь).
        Известный рассинхрон схемы: ниже ищутся поля 'Тип_кольца'/'ring_type' и
        'Индекс_кольца'/'ring_index', но реальный creator точечных слоёв
        Fsm_2_1_6._create_fields пишет 'Тип_контура' (значения 'Внешний'/'Внутренний')
        и 'Номер_контура'. При точечном входе тип/индекс кольца НЕ прочитаются (тихий
        fallback exterior/0). Если продукт когда-нибудь станет подавать Т-слой напрямую
        как layer — согласовать имена полей И значения ('hole' vs 'Внутренний') с
        Fsm_2_1_6._create_fields перед использованием.
