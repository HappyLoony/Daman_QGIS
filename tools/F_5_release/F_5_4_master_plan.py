...
Результат: набор отдельных PDF-файлов в папке экспорта (по одному на схему).
Имя каждого файла берётся из `Base_drawings.pdf_filename` (с fallback на
legacy `{NN}_{drawing_name}.pdf`).
"""
...
class F_5_4_MasterPlan(BaseTool):
    ...
    def run(self) -> None:
        ...
        try:
            ...
            # Темы НЕ удаляются: они нужны сохранённым макетам для
            ...
            # Открытие папки с результатами (вместо склееного PDF — каждая
            # схема сохраняется как отдельный файл, имя берётся из
            # Base_drawings.pdf_filename).
            log_success(
                f"F_5_4: Создано {len(pdf_paths)} отдельных PDF в {output_folder}"
            )
            self.iface.messageBar().pushMessage(
                "Мастер-план",
                f"Создано {len(pdf_paths)} отдельных PDF в {path_for_display(output_folder)}",
                level=Qgis.MessageLevel.Info,
                duration=5
            )
            self._open_result(output_folder)
    ...
    def _generate_single_scheme(
        ...
        try:
            ...
            # l. Экспорт PDF — имя файла берётся из Base_drawings.pdf_filename
            # (без расширения, .pdf добавляется здесь). Если поле не задано,
            # fallback на legacy формат "{NN}_{drawing_name}.pdf".
            custom_name = drawing.get('pdf_filename')
            if custom_name:
    ...
    def _open_result(self, path: str) -> None:
        """
        Открыть путь (файл или папку) в системном просмотрщике/файловом менеджере.

        Args:
            path: Путь к файлу или папке. На Windows os.startfile, на macOS
                /usr/bin/open, на Linux xdg-open — все поддерживают
                и файлы, и папки.
        """
        try:
            import subprocess
            ...
            log_info(f"F_5_4: Открыт путь: {path}")
        except Exception as e:
            log_warning(f"F_5_4: Не удалось открыть путь: {e}")
            self.iface.messageBar().pushMessage(
                "Мастер-план",
                f"Результат: {path_for_display(path)}",
                level=Qgis.MessageLevel.Info,
                duration=10
            )
