...
# ===========================================================================
# Уведомление пользователей (msg command)
# ===========================================================================
def notify_users(
    locked_files: list,
    folder_name: str,
) -> int:
    """Отправить сообщение пользователям о необходимости закрыть файлы.

    Использует команду msg * /server:HOSTNAME для отправки
    на рабочий стол пользователя.

    Args:
        locked_files: список LockedFile с информацией о блокировках
        folder_name: имя папки (для текста сообщения)
    ...
    # Собрать уникальные хосты и файлы
    host_files: Dict[str, List[str]] = {}
    for lf in locked_files:
        host = lf.locked_by_host
        if not host or host == "--":
            continue
        # Пропустить свой ПК
        local_pc = os.environ.get("COMPUTERNAME", "").upper()
        if host.upper() == local_pc:
            continue
        if host not in host_files:
            host_files[host] = []
        host_files[host].append(lf.file_name)

    if not host_files:
        log_info("Fsm_6_5_3: notify_users: нет хостов для уведомления")
        return 0
    ...
    for host, files in host_files.items():
        ...
        if len(files) > 5:
            file_list += f" (и ещё {len(files) - 5})"

        message = (
            f"Пожалуйста, закройте файлы в папке '{folder_name}':\n"
            f"{file_list}\n\n"
            f"Требуется переименование/архивация папки."
        )

        try:
            result = _sp.run(
                ["msg", "*", f"/server:{host}", message],
                capture_output=True,
                timeout=10,
            )
            if result.returncode == 0:
                sent += 1
                log_info(
                    f"Fsm_6_5_3: msg sent to {host}: "
                    f"{len(files)} файлов"
                )
            else:
                log_warning(
                    f"Fsm_6_5_3: msg to {host} failed: "
                    f"rc={result.returncode}"
                )
        except (OSError, _sp.TimeoutExpired) as exc:
            log_warning(f"Fsm_6_5_3: msg to {host} error: {exc}")
