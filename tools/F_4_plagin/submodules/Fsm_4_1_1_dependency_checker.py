# -*- coding: utf-8 -*-
"""
Fsm_4_1_1_DependencyChecker - Проверка Python зависимостей
...
class DependencyChecker:
    ...
    PACKAGE_DESCRIPTIONS = {
        ...
        'comtypes': {
            'description': 'COM автоматизация для Windows (Word, Excel, AutoCAD)',
            'usage': 'Конвертация DWG/DOCX/XLSX в PDF (F_6_2)'
        },
        'pypdf': {
    ...
    def get_requirements_path(cls) -> str:
        ...
        # Fsm_4_1_1_...py -> submodules -> F_4_plagin -> tools -> Daman_QGIS
        current_file = os.path.dirname(__file__)  # submodules/
        f5_plagin = os.path.dirname(current_file)  # F_4_plagin/
    ...
    def parse_requirements(cls) -> List[Tuple[str, Optional[str], str, bool]]:
        ...
        if not os.path.exists(requirements_path):
            log_warning(f"F_4_1: requirements.txt не найден: {requirements_path}")
            return requirements
    ...
    def get_pypi_version(cls, package_name: str) -> Optional[str]:
        ...
        try:
            ...
            log_warning(f"F_4_1: Не удалось получить версию {package_name} из PyPI: {e.reason}")
        except urllib.error.HTTPError as e:
            log_warning(f"F_4_1: HTTP ошибка при запросе {package_name}: {e.code}")
        except json.JSONDecodeError:
            log_warning(f"F_4_1: Некорректный JSON от PyPI для {package_name}")
        except Exception as e:
            log_warning(f"F_4_1: Ошибка при запросе PyPI для {package_name}: {str(e)}")
    ...
    def check_for_update(cls, package_name: str, installed_version: Optional[str]) -> Tuple[bool, Optional[str]]:
        ...
        try:
            ...
            log_warning(f"F_4_1: Ошибка сравнения версий {package_name}: {str(e)}")
    ...
    def get_version_from_dependencies(cls, package_name: str) -> Optional[str]:
        ...
        """
        try:
            # Получаем путь к dependencies напрямую без импорта PipInstaller
            # чтобы избежать возможных побочных эффектов при импорте
            from qgis.core import QgsApplication
            qgis_settings_dir = QgsApplication.qgisSettingsDirPath().replace("/", os.path.sep)
            dependencies_path = os.path.join(qgis_settings_dir, "python", "dependencies")
            ...
            log_warning(f"Fsm_4_1_1: Ошибка получения версии из dependencies для {package_name}: {e}")
            return None
    ...
    def check_dependency(cls, module_name: str, min_version: Optional[str] = None) -> Tuple[bool, Optional[str], str]:
        ...
        if installed_version is None:
            ...
            try:
                ...
                # Пробуем оба варианта имени (с - и с _)
                package_names_to_try = [module_name]
                if '-' in module_name:
                    package_names_to_try.append(module_name.replace('-', '_'))
        ...
        if min_version and installed_version:
            try:
                ...
                log_warning(f"F_4_1: packaging не установлен, пропускаем проверку версии {module_name}")
            except Exception as e:
                log_warning(f"F_4_1: Ошибка сравнения версий {module_name}: {str(e)}")
    ...
    def quick_check(cls) -> bool:
        ...
        Использует importlib.metadata для проверки БЕЗ импорта модулей,
        чтобы избежать побочных эффектов от выполнения кода модулей.
        ...
        for module_name in required_deps.keys():
            ...
            # Сначала проверяем в папке dependencies (приоритет)
            version = cls.get_version_from_dependencies(module_name)

            # Если не нашли в dependencies, проверяем через importlib.metadata
            if version is None:
                # Пробуем оба варианта имени (с - и с _)
                package_names_to_try = [module_name]
                if '-' in module_name:
        ...
        if not missing:
            ...
            if installed:
                log_info(f"F_4_1: Установлены: {', '.join(installed_with_versions)}")
            all_ok = False

        # Краткая сводка среды
        try:
            from .Fsm_4_1_14_environment_doctor import EnvironmentDoctor
            env_summary = EnvironmentDoctor.quick_summary()
            log_info(f"F_4_1: Среда: {env_summary}")
        except Exception:
            pass
