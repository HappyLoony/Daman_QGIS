# -*- coding: utf-8 -*-
"""
Fsm_4_2_T_signals - Тест Qt сигналов и слотов
...
class TestSignals:
    ...
    def test_01_project_layer_signals(self) -> None:
        ...
        # Сохраняем слоты для корректного disconnect
        slot_added = self.catcher.catch_signal('layersAdded')
        slot_removed = self.catcher.catch_signal('layersRemoved')

        try:
            # Подключаем сигналы
            project.layersAdded.connect(slot_added)
            project.layersRemoved.connect(slot_removed)
            ...
            # Отключаем ТОЛЬКО наши слоты (не затрагивая другие подключения)
            try:
                project.layersAdded.disconnect(slot_added)
            except Exception:
                pass
            try:
                project.layersRemoved.disconnect(slot_removed)
            except Exception:
                pass
    ...
    def test_02_layer_feature_signals(self) -> None:
        ...
        try:
            ...
            # Подключаем сигналы (сохраняем ссылку на слот)
            slot_feature_added = self.catcher.catch_signal('featureAdded')
            layer.featureAdded.connect(slot_feature_added)
            ...
            # Очистка
            layer.featureAdded.disconnect(slot_feature_added)
            del layer
    ...
    def test_03_layer_edit_signals(self) -> None:
        ...
        try:
            ...
            # Подключаем сигналы (сохраняем ссылки на слоты)
            slot_started = self.catcher.catch_signal('editingStarted')
            slot_stopped = self.catcher.catch_signal('editingStopped')
            layer.editingStarted.connect(slot_started)
            layer.editingStopped.connect(slot_stopped)
            ...
            # Отключаем сигналы
            layer.editingStarted.disconnect(slot_started)
            layer.editingStopped.disconnect(slot_stopped)
            del layer
    ...
    def test_04_canvas_signals(self) -> None:
        ...
        self.catcher.reset()
        slot_extents = self.catcher.catch_signal('extentsChanged')
        ...
        try:
            ...
            # Подключаем сигналы
            canvas.extentsChanged.connect(slot_extents)
            ...
            # Отключаем ТОЛЬКО наш слот
            canvas.extentsChanged.disconnect(slot_extents)
    ...
    def test_06_block_signals(self) -> None:
        ...
        try:
            ...
            # Подключаем сигнал (сохраняем ссылку на слот)
            slot_editing = self.catcher.catch_signal('editingStarted')
            layer.editingStarted.connect(slot_editing)
            ...
            layer.editingStarted.disconnect(slot_editing)
            del layer
