# -*- coding: utf-8 -*-
"""
Fsm_4_2_T_plugin_lifecycle - Тест жизненного цикла плагина
...
class TestPluginLifecycle:
    ...
    def test_06_managers_initialized(self) -> None:
        ...
        # Классы проверяются через импорт из managers (новая архитектура с доменами)
        managers_to_check = [
            'ProjectManager',
            'LayerManager',
            'ReferenceManagers',
        ...
        try:
            from Daman_QGIS import managers
            for class_name in managers_to_check:
                manager_class = getattr(managers, class_name, None)
                if manager_class:
                    self.logger.success(f"{class_name} импортируется")
                else:
                    self.logger.fail(f"{class_name} не найден в managers!")
        except ImportError as e:
            self.logger.fail(f"Ошибка импорта managers: {e}")
