...
class TestFgislkMonitoring:
    ...
    def _test_08_changelog_rosleshoz(self):
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                has_version = bool(re.search(r"фгис\s+лк[:\s]+\d+\.\d+", text)) or "версия" in text
    ...
    def _test_26_changelog_deep_parse(self):
        """ТЕСТ 26: Changelog -- контрактная проверка структуры и серии ФГИС-ЛК."""
        self.logger.section("26. Changelog: контракт структуры и серии")
        ...
        try:
            ...
            for phrase in FGISLK_CHANGELOG_CONTRACT["required_phrases"]:
                self.logger.check(
                    phrase in text_lower,
                    f"Структура: фраза '{phrase}' присутствует",
                    f"Структура изменилась: отсутствует ожидаемая фраза '{phrase}'"
                )
            ...
            if actual_series is None:
                self.logger.fail("Серия ФГИС-ЛК не найдена в релизных строках -- структура страницы изменилась радикально")
            else:
                self.logger.check(
                    actual_series == expected_series,
                    f"Серия ФГИС-ЛК совпадает с контрактом: {actual_series}",
                    f"Серия сменилась: контракт {expected_series}, фактическая {actual_series} -- "
                    f"проверить endpoint gwc-01/02, LAYER_MAPPING (Fsm_1_2_4), схему PBF"
                )
            self.logger.data("Expected серия (контракт)", expected_series)
            self.logger.data("Actual серия (changelog)", actual_series or "не найдена")

            # Релевантные ключевые слова -- sanity check контента (тайловый сервис)
            keywords = {
            ...
            self.logger.data("Релевантных ключевых слов", f"{found_count}/{len(keywords)}")

            # Обращение 523042 (инцидент смены тайлового endpoint, 2026-03): исторический маркер
            self.logger.data("Обращение 523042 (тайлы)", "Найдено" if "523042" in text else "Не найдено (норма для архивации)")
