# -*- coding: utf-8 -*-
"""
Субмодуль Fsm_4_2_T_3_5 - Тест функции F_2_5_Изъятие
...
- Структуру диалога Fsm_2_5_1_Dialog
- Логику переноса Fsm_2_5_2_Transfer
"""
...
class TestF35:
    ...
    def run_all_tests(self):
        self.logger.section("ТЕСТ F_2_5: Изъятие ЗУ")
    ...
    def test_01_import_module(self):
        """ТЕСТ 1: Импорт основного модуля F_2_5"""
        self.logger.section("1. Импорт F_2_5_Izyatie")
        try:
            from Daman_QGIS.tools.F_2_cutting.F_2_5_izyatie import F_2_5_Izyatie
            self.module = F_2_5_Izyatie(self.iface)
            self.logger.success("Модуль F_2_5_Izyatie загружен")
    ...
    def test_02_check_structure(self):
        ...
        self.logger.section("2. Структура класса F_2_5_Izyatie")
    ...
    def test_03_check_submodules(self):
        ...
        self.logger.section("3. Субмодули F_2_5")

        # Fsm_2_5_1_Dialog
        try:
            from Daman_QGIS.tools.F_2_cutting.submodules.Fsm_2_5_1_dialog import (
                Fsm_2_5_1_Dialog
            )
            self.logger.success("Fsm_2_5_1_Dialog: импорт OK")
        except ImportError as e:
            self.logger.fail(f"Fsm_2_5_1_Dialog: ошибка импорта - {e}")

        # Fsm_2_5_2_Transfer
        try:
            from Daman_QGIS.tools.F_2_cutting.submodules.Fsm_2_5_2_transfer import (
                Fsm_2_5_2_Transfer
            )
            self.logger.success("Fsm_2_5_2_Transfer: импорт OK")
        except ImportError as e:
            self.logger.fail(f"Fsm_2_5_2_Transfer: ошибка импорта - {e}")
    ...
    def test_05_check_dialog(self):
        """ТЕСТ 5: Проверка структуры диалога Fsm_2_5_1_Dialog"""
        self.logger.section("5. Структура Fsm_2_5_1_Dialog")
        ...
        try:
            ...
            for method in dialog_methods:
                if hasattr(Fsm_2_5_1_Dialog, method):
                    self.logger.success(f"Метод {method}: найден")
                else:
            ...
            if issubclass(Fsm_2_5_1_Dialog, QDialog):
    ...
    def test_06_check_transfer(self):
        """ТЕСТ 6: Проверка структуры Fsm_2_5_2_Transfer"""
        self.logger.section("6. Структура Fsm_2_5_2_Transfer")
        ...
        try:
            ...
            for method in transfer_methods:
                if hasattr(Fsm_2_5_2_Transfer, method):
                    self.logger.success(f"Метод {method}: найден")
                else:
                    self.logger.fail(f"Метод {method}: НЕ найден")
            ...
            import inspect
            source_code = inspect.getsource(Fsm_2_5_2_Transfer)
