# -*- coding: utf-8 -*-
"""
Субмодуль Fsm_4_2_1_1 - Логгер тестов
...
class TestLogger:
    ...
    def skip(self, message: str):
        """Пропуск теста (считается как warning, показывается при LOG_LEVEL_WARNING и LOG_LEVEL_ALL)"""
        self.warning_count += 1
        if self.log_level <= self.LOG_LEVEL_WARNING:
            context = f"[{self._current_section}] " if self._current_section else ""
            self.log_lines.append(f"  SKIP: {context}{message}")
    ...
    def data(self, key: str, value: str):
        """Вывод данных (при LOG_LEVEL_WARNING и ниже, или всегда для error контекста)"""
        if self.log_level <= self.LOG_LEVEL_WARNING:
            self.log_lines.append(f"    {key}: {value}")

    def summary(self):
        """Итоговая сводка (только при LOG_LEVEL_WARNING и ниже)"""
