# -*- coding: utf-8 -*-
"""
Субмодуль Fsm_4_2_T_5_1 - Тест функции F_4_1_Проверка зависимостей
...
class TestF51:
    """Тесты для функции F_4_1_PluginDiagnostics"""
    ...
    def run_all_tests(self):
        """Запуск всех тестов F_4_1"""
        self.logger.section("ТЕСТ F_4_1: Проверка зависимостей")
        ...
        try:
            ...
            self.logger.error(f"Критическая ошибка тестов F_4_1: {str(e)}")

        self.logger.summary()

    def test_01_init_module(self):
        """ТЕСТ 1: Инициализация модуля F_4_1"""
        self.logger.section("1. Инициализация F_4_1_PluginDiagnostics")

        try:
            from Daman_QGIS.tools.F_4_plagin.F_4_1_plugin_diagnostics import F_4_1_PluginDiagnostics

            self.module = F_4_1_PluginDiagnostics(self.iface)
            self.logger.success("Модуль F_4_1_PluginDiagnostics загружен")

            # Проверяем наличие свойства name
            if hasattr(self.module, 'name'):
                name = self.module.name
                self.logger.check(
                    "4_1" in name or "иагностик" in name.lower(),
