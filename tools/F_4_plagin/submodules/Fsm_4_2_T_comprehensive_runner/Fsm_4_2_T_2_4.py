# -*- coding: utf-8 -*-
"""
Субмодуль Fsm_4_2_T_3_4 - Тест функции F_2_4_Этапность
Проверка формирования этапов кадастровых работ (1 этап, 2 этап, итог)
"""


class TestF34:
    """Тесты для F_2_4_Staging (этапность)"""
    ...
    def run_all_tests(self):
        ...
        self.logger.section("ТЕСТ F_2_4: Этапность")
        ...
        try:
            ...
            self.test_07_object_type_check()
    ...
    def test_01_init_module(self):
        """ТЕСТ 1: Инициализация модуля F_2_4_Staging"""
        self.logger.section("1. Инициализация F_2_4_Staging")

        try:
            from Daman_QGIS.tools.F_2_cutting.F_2_4_staging import F_2_4_Staging

            self.module = F_2_4_Staging(self.iface)
            self.logger.success("Модуль F_2_4_Staging загружен")
    ...
    def test_07_object_type_check(self):
        """ТЕСТ 7: Проверка блокировки для линейных объектов"""
        self.logger.section("7. Проверка _check_object_type")
        ...
        try:
            # Проверяем наличие метода _check_object_type
            self.logger.check(
                hasattr(self.module, '_check_object_type'),
                "Метод _check_object_type существует",
                "Метод _check_object_type отсутствует!"
            )

            # Проверяем что ProjectDB доступен
            from Daman_QGIS.database.project_db import ProjectDB
            self.logger.success("ProjectDB доступен для проверки типа объекта")

            # Проверяем что ProjectSettings имеет поле object_type
            from Daman_QGIS.database.schemas import ProjectSettings
            self.logger.check(
                hasattr(ProjectSettings, 'object_type'),
                "ProjectSettings.object_type существует",
                "ProjectSettings.object_type отсутствует!"
            )

            # Проверяем на текущем проекте (если открыт)
            from Daman_QGIS.managers import registry
            ...
            if gpkg_path:
                ...
                if settings:
                    obj_type = settings.object_type
                    self.logger.data("Тип объекта в проекте", obj_type or "(пусто)")

                    self.logger.check(
                        obj_type in ("Площадной", "Линейный", ""),
                        f"Тип объекта корректный: '{obj_type}'",
                        f"Неизвестный тип объекта: '{obj_type}'"
                    )

                    if obj_type == "Линейный":
                        self.logger.success(
                            "Проект линейный - F_2_4 должен быть заблокирован"
                        )
                    elif obj_type == "Площадной":
                        self.logger.success(
                            "Проект площадной - F_2_4 доступен"
                        )
                else:
                    self.logger.data("Настройки проекта", "не загружены")
            else:
                self.logger.data("GeoPackage", "не найден (проект не открыт)")

        except Exception as e:
            self.logger.error(f"Ошибка проверки типа объекта: {str(e)}")
            import traceback
            self.logger.data("Traceback", traceback.format_exc())
