...
- Calibration: трансформация object через исходную СК
- LDP: использование RAW координат объектов
...
class Fsm_0_5_4_2_OffsetMeridian(BaseCalculationMethod):
    ...
    def calculate_ldp(
        ...
        if len(raw_wrong_coords) < self.min_points:
            log_error(f"Fsm_0_5_4_2 [LDP]: Нужно минимум {self.min_points} точек")
            return None
        ...
        if use_direct_3857 and len(raw_wrong_coords) != len(reference_3857):
            log_error("Fsm_0_5_4_2 [LDP]: Несоответствие количества точек (raw vs 3857)")
            return None
        elif not use_direct_3857 and len(raw_wrong_coords) != len(control_points_wgs84):
            log_error("Fsm_0_5_4_2 [LDP]: Несоответствие количества точек")
            return None
        ...
        try:
            ...
            if object_center_lon is None:
                log_error("Fsm_0_5_4_2 [LDP]: object_center_lon не задан, используем initial_lon_0")
            ...
            if final_rmse == float('inf'):
                log_error(f"Fsm_0_5_4_2 [LDP]: Невалидная CRS для lon_0={fixed_lon_0}")
                return None
            ...
            status = "OK" if success else "WARN"
            log_info(
                f"Fsm_0_5_4_2 [{self.method_id}] [LDP]: "
            ...
            log_error(f"Fsm_0_5_4_2 [LDP]: Ошибка расчёта: {e}")
            return None

    # _calc_offset, _calc_offset_ldp, _golden_section_search наследуются из BaseCalculationMethod
