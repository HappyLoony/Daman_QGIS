...
# В метаданных `1_2_object_type` хранится КОД, а не отображаемое название
# (название лежит отдельно в `1_2_object_type_name`). Сравнивать тип объекта
# с русской строкой нельзя — условие не выполнится никогда.
# Проверка типа — единственным хелпером `ProjectManager.is_linear_object()`.
OBJECT_TYPE_AREA = 'area'      # Площадной объект
OBJECT_TYPE_LINEAR = 'linear'  # Линейный объект

# Значение поля «Точки» для контуров без нумерации характерных точек
# (Без_Меж, Изм). Дом конвенции: прочерк, единый для всех писателей пакета
# F_2. Пустая строка неотличима от «поле не заполнено» и ломает сверку
# состава координатного перечня.
POINTS_FIELD_NONE = "-"
...
# Допуск склейки дублирующихся вершин. Строго МЕНЬШЕ шага кадастровой сетки:
# `removeDuplicateNodes` сравнивает координаты покомпонентно с этим допуском,
# поэтому при значении, равном шагу сетки, снимались бы легитимные вершины,
# отстоящие ровно на один шаг.
VERTEX_DUPLICATE_EPSILON = COORDINATE_PRECISION / 10.0  # 1 миллиметр
...
# Обобщённая формулировка, когда конкретная причина изменения не определена
