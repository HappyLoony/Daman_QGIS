...
Назначение:
    ...
    выборки ЗУ в слои нарезки (Le_2_1_*) и этапности (Le_2_7_*).

Логика:
    1. Для слоёв 1 этапа (Le_2_1_*): синхронизация по полю КН
    2. Для слоёв 2 этапа и итоговых (Le_2_7_* с полем Состав_контуров):
...
from Daman_QGIS.constants import LAYER_CUTTING_PREFIX, LAYER_STAGING_PREFIX
...
class CuttingSync:
    ...
    TARGET_PREFIXES = (LAYER_CUTTING_PREFIX, LAYER_STAGING_PREFIX)
    ...
    def sync_cutting_layers(
        ...
        Находит все слои Le_2_1_* (нарезка) и Le_2_7_* (этапность),
        ...
        # Ищем слои нарезки Le_2_1_* и этапности Le_2_7_*
        cutting_layers = [
        ...
        if not cutting_layers:
            log_info("Msm_23_3: Слои нарезки Le_2_1_* и этапности Le_2_7_* не найдены")
            return stats
    ...
    def _sync_single_cutting_layer(
        ...
        Для слоёв 1 этапа (Le_2_1_*) - синхронизация по полю КН.
        Для слоёв 2 этапа и итоговых (Le_2_7_*, с полем Состав_контуров != '-'):
    ...
    def _collect_stage1_oks_data(self) -> Dict[int, Dict[str, str]]:
        ...
        Ищет слои Le_2_1_* и собирает данные по полю ID.
        ...
        stage1_layers = [
            layer for layer in QgsProject.instance().mapLayers().values()
            if isinstance(layer, QgsVectorLayer) and layer.name().startswith('Le_2_1_')
        ]
