...
def _get_project_structure_manager():
...
def add_layer_to_project(
    ...
    try:
        ...
        if gpkg_path and os.path.exists(gpkg_path):
            ...
            # Удаляем memory-версию (если была добавлена в проект)
            # ПРИМЕЧАНИЕ: memory-слой из create_memory_layer() НЕ добавлен в проект,
            # поэтому проверяем наличие перед удалением
            ...
            if gpkg_layer.isValid():
                if layer_manager:
                    ...
                    log_warning(f"Msm_25_0: layer_manager=None, добавление напрямую через project.addMapLayer()")
