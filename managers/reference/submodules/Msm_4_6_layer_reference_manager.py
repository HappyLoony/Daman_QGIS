...
class LayerReferenceManager(BaseReferenceLoader):
    ...
    def get_graphics_selectable_layers(self) -> List[Dict]:
        """
        Получить слои доступные для выбора в графике (F_1_4)

        Слои с graphics_selectable=1 могут быть выбраны пользователем
        в диалоге F_1_4 для включения в схему.

        Returns:
            Список слоёв с graphics_selectable=1
        """
        layers = self.get_base_layers()
        return [
            layer for layer in layers
            if layer.get('graphics_selectable') == 1
        ]
