...
from .submodules.Msm_12_2_label_settings_builder import LabelSettingsBuilder
from .submodules.Msm_12_1_collision_manager import CollisionManager
from Daman_QGIS.utils import log_info, log_warning, log_error

# Lazy imports для избежания циклических зависимостей
# LabelReferenceManager, ExpressionManager импортируются внутри методов
...
class LabelManager:
    ...
    def __init__(self):
        ...
        self._expr_manager = None
        self._label_ref_manager = None
        self._all_ref_managers = None
        self._settings_builder = None
    ...
    def expr_manager(self):
        """Ленивая инициализация ExpressionManager"""
        if self._expr_manager is None:
            from Daman_QGIS.managers import ExpressionManager
            self._expr_manager = ExpressionManager()
        return self._expr_manager

    @property
    def label_ref_manager(self):
        """Ленивая инициализация LabelReferenceManager"""
        if self._label_ref_manager is None:
            from Daman_QGIS.managers import LabelReferenceManager
            self._label_ref_manager = LabelReferenceManager()
        return self._label_ref_manager

    @property
    def all_ref_managers(self):
        """Ленивая инициализация всех справочных менеджеров"""
        if self._all_ref_managers is None:
            from Daman_QGIS.managers import get_reference_managers
            self._all_ref_managers = get_reference_managers()
        return self._all_ref_managers

    @property
    def settings_builder(self):
        """Ленивая инициализация LabelSettingsBuilder"""
        if self._settings_builder is None:
            layers_ref_manager = self.all_ref_managers.layer
            self._settings_builder = LabelSettingsBuilder(
                expr_manager=self.expr_manager,
                label_ref_manager=self.label_ref_manager,
                layers_ref_manager=layers_ref_manager
            )
        return self._settings_builder
    ...
    def apply_labels(self, layer: QgsVectorLayer, full_name: str) -> bool:
        ...
        try:
            ...
            label_config = self.label_ref_manager.get_label_config(full_name)
            ...
            defaults = self.label_ref_manager.get_default_config()
            ...
            # Передаём layer для автоопределения типа препятствия по стилю
    ...
    def has_labels_configured(self, full_name: str) -> bool:
        ...
        """
        return self.label_ref_manager.has_labels_enabled(full_name)
    ...
    def get_label_info(self, full_name: str) -> Optional[dict]:
        ...
        """
        config = self.label_ref_manager.get_label_config(full_name)
        if config:
            # Дополняем defaults
            defaults = self.label_ref_manager.get_default_config()
    ...
    def get_layers_with_labels(self) -> list:
        ...
        """
        return self.label_ref_manager.get_layers_with_labels()
