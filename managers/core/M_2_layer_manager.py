...
# CoordinatePrecisionManager, StyleManager, LayerReplacementManager,
# get_reference_managers, LabelManager - импортируются внутри методов
...
class LayerManager:
    ...
    def style_manager(self):
        """Ленивая инициализация менеджера стилей"""
    ...
    def replacement_manager(self):
        """Ленивая инициализация менеджера замены слоёв"""
        if self._replacement_manager is None:
            from Daman_QGIS.managers import LayerReplacementManager
            self._replacement_manager = LayerReplacementManager()
        return self._replacement_manager

    @property
    def label_manager(self):
        """Ленивая инициализация менеджера подписей"""
    ...
    def label_manager(self, value):
        """Setter для совместимости с существующим кодом"""
    ...
    def add_layer(self,
        ...
        if isinstance(layer, QgsVectorLayer):
            try:
                self.label_manager.apply_labels(layer, layer_name)
        ...
        if isinstance(layer, QgsVectorLayer):
            ...
            # Безопасное обновление отображения через QTimer
        ...
        # Безопасное обновление canvas после добавления слоя
        from Daman_QGIS.utils import safe_refresh_canvas, REFRESH_MEDIUM
