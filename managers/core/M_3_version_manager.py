...
from .M_19_project_structure_manager import FolderType, PROJECT_FOLDERS, RELEASE_PATTERN

# Lazy import для избежания циклических зависимостей
# registry импортируется внутри методов
...
class VersionManager:
    ...
    def __init__(self, iface: Any, project_path: Optional[str] = None) -> None:
        ...
        """
        self.iface = iface
        self.__structure_manager = None  # Lazy init
        self._init_project_path = project_path

    def _get_structure_manager(self):
        """Ленивая инициализация M_19"""
        if self.__structure_manager is None:
            from Daman_QGIS.managers import registry
            self.__structure_manager = registry.get('M_19')
            if self._init_project_path:
                self.__structure_manager.project_root = self._init_project_path
        return self.__structure_manager

    @property
    def project_path(self) -> Optional[str]:
        """Путь к проекту"""
        return self._get_structure_manager().project_root

    @project_path.setter
    def project_path(self, value: Optional[str]):
        """Установка пути к проекту"""
        self._get_structure_manager().project_root = value

    def set_project_path(self, project_path: str) -> None:
        """Установка пути к проекту"""
        self._get_structure_manager().project_root = project_path
    ...
    def _get_working_path(self) -> Optional[str]:
        ...
        return self._get_structure_manager().get_folder(FolderType.WORKING, create=False)

    def _get_releases_path(self) -> Optional[str]:
        """Получить путь к папке выпусков"""
        return self._get_structure_manager().get_folder(FolderType.RELEASES, create=True)
    ...
    def create_version(self, description: str = "", author: str = "") -> Optional[str]:
        ...
        """
        if not self._get_structure_manager().is_active():
