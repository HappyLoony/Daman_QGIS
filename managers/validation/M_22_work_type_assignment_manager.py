...
- NGS (вне существующего ЗУ): "Образование земельного участка из земель, находящихся
  в государственной или муниципальной собственности" (ЗК РФ ст. 11.2 ч. 1 / ст. 11.3 —
  «из земель», исходного участка у НГС нет)
...
Используется в:
- F_2_1 (нарезка) - базовая нарезка
- F_2_3 (корректировка) - корректировка контуров
- F_2_4 (этапность) - этапы 1 и 2
"""
...
__all__ = ['LayerType', 'StageType', 'WorkTypeAssignmentManager']
...
class WorkTypeAssignmentManager:
    ...
    def _load_databases(self) -> bool:
        ...
        from Daman_QGIS.managers import SimpleReferenceManager
    ...
    def _get_vri_manager(self) -> 'VRIAssignmentManager':
        """Получить экземпляр VRIAssignmentManager через registry
        ...
        if self._vri_manager is None:
            ...
            self._vri_manager = registry.get('M_21')
    ...
    def _get_work_type_value(
        ...
        Args:
            ...
            stage_type: Тип этапа (для F_2_4)
        ...
        # NGS (из земель гос./мун. собственности).
        # «из ЗЕМЕЛЬ», не «из земельных участков»: у НГС исходного участка не
        # существует — привязка идёт к кадастровому кварталу (Msm_26_5).
        # Классификатор Росреестра dMethod (dMethod_v02.xsd, схема XML утв. ПМТ)
        # разводит способы по кодам: 2 «Раздел» — это наш RAZDEL, 5 «Образование
        # из земель» — этот случай. Оборот «из земель, находящихся в
        # государственной или муниципальной собственности» — дословно ЗК ст. 11.2
        # ч. 1. Заголовок ЗК ст. 11.3 («из земель ИЛИ земельных участков») —
        # родовой для всей статьи, для конкретного ЗУ не подходит.
        # Значение обязано совпадать с vedomost_value справочника Work_types
        # (планка: scripts/check_work_types_sync.py).
    ...
    def assign_work_type_basic(
        ...
        """Присвоить Вид_Работ для базовой нарезки (F_2_1, F_2_3)
    ...
    def assign_work_type_stage1(
        ...
        """Присвоить Вид_Работ для этапа 1 (F_2_4)
    ...
    def assign_work_type_stage2(
        ...
        """Присвоить Вид_Работ для этапа 2 - объединение (F_2_4)
        ...
        for item in features_data:
            ...
            if not merged_ids:
                # Пробуем получить из merged_contours (F_2_4 передаёт так)
