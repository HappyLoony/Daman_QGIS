...
Паттерн: Singleton с методом reload() для обновления кэша без перезапуска QGIS.
Использование: VRIAssignmentManager.get_instance()

Используется в:
- F_2_1 (нарезка) - итоговые слои
- F_2_3 (ЗПР ОКС) - итоговые слои
- F_2_4 (этапность) - только слой "Итог"
"""
...
class VRIAssignmentManager:
    """Менеджер присвоения и валидации ВРИ (Singleton)

    Использование:
        manager = VRIAssignmentManager.get_instance()
        # или для принудительной перезагрузки кэша:
        manager = VRIAssignmentManager.get_instance(force_reload=True)
    """

    # Singleton instance и lock для thread-safety
    _instance: Optional['VRIAssignmentManager'] = None
    ...
    def get_instance(cls, force_reload: bool = False) -> 'VRIAssignmentManager':
        ...
        Args:
            force_reload: Если True - принудительно перезагрузить данные VRI.json

        Returns:
            Экземпляр VRIAssignmentManager
        """
        with cls._lock:
            if cls._instance is None:
                cls._instance = cls.__new__(cls)
                cls._instance._init_instance()
                log_info("M_21: Singleton instance created")
    ...
    def _init_instance(self) -> None:
        """Инициализация экземпляра (вызывается один раз)"""
        ...
        self._vri_by_name: Dict[str, Dict] = {}  # Индекс по name (короткое имя)
    ...
    def __init__(self, plugin_dir: Optional[str] = None) -> None:
        ...
        """
        # Проверяем, был ли уже инициализирован через Singleton
    ...
    def reload(self) -> bool:
        """Принудительно перезагрузить данные VRI.json

        Очищает кэш и загружает данные заново.
        Полезно когда VRI.json был обновлён без перезапуска QGIS.

        Returns:
            True если перезагрузка успешна
        """
        log_info("M_21: Принудительная перезагрузка VRI.json...")
        self._vri_data = []
        self._vri_by_full_name = {}
        self._vri_by_name = {}
        self._vri_by_code = {}
        self._loaded = False
        return self._load_vri_database()
    ...
    def _validate_single_vri(self, vri_str: str) -> bool:
        ...
        Поддерживает поиск по full_name, name и code.
        ...
        # Поиск по name (короткое имя)
        if normalized in self._vri_by_name:
            return True
    ...
    def _get_vri_data_for_single(self, vri_str: str) -> Optional[Dict]:
        ...
        # 2. Поиск по name (короткое имя без кода)
        # Важно для Без_Меж слоёв, где ВРИ из исходного ЗУ может быть в формате "Отдых (рекреация)"
        if normalized in self._vri_by_name:
            return self._vri_by_name[normalized]

        # 3. Поиск по коду (без нормализации регистра)
        if cleaned in self._vri_by_code:
    ...
    def reassign_vri_by_geometry(
        ...
        Используется в F_2_3 (Корректировка) когда пользователь мог изменить ЗПР
