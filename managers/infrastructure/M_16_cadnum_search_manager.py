# -*- coding: utf-8 -*-
"""
CadnumSearchManager - Поиск объектов по кадастровому номеру и номеру ЗОУИТ
...
class CadnumSearchManager:
    ...
    # Регулярное выражение для валидации номера ЗОУИТ
    # Формат: XX:XX-X.X+
    # Регион: 1-2 цифры, Район: 1-2 цифры, дефис, номер с точкой
    # Примеры: 89:11-6.1612, 89:11-6.15
    ZOUIT_PATTERN = re.compile(
        r'^\d{1,2}:\d{1,2}-\d+\.\d+$'
    )
    ...
    def init_gui(self):
        ...
        self.action = QAction("Поиск по номеру (КН / ЗОУИТ)...", self.iface.mainWindow())
        if self.action:
            self.action.setToolTip("Поиск объектов по кадастровым номерам и номерам ЗОУИТ во всех слоях проекта")
    ...
    @staticmethod
    def validate_zouit_number(number: str) -> bool:
        """
        Валидация формата номера ЗОУИТ

        Формат: XX:XX-X.X+
        Регион (1-2): Район (1-2) - номер.подномер
        ...
        Args:
            number: Номер ЗОУИТ для проверки

        Returns:
            bool: True если формат корректен, False иначе
        """
        number = number.strip()
        return bool(CadnumSearchManager.ZOUIT_PATTERN.match(number))

    @staticmethod
    def classify_identifier(text: str) -> str:
        """
        Классификация введённого идентификатора

        Args:
            text: Строка для классификации

        Returns:
            str: 'cadnum' | 'zouit' | 'unknown'
        """
        text = text.strip()
        if CadnumSearchManager.CADNUM_PATTERN.match(text):
            return 'cadnum'
        elif CadnumSearchManager.ZOUIT_PATTERN.match(text):
            return 'zouit'
        return 'unknown'

    @staticmethod
    def validate_identifier(text: str) -> bool:
        """
        Проверка что идентификатор является валидным КН или номером ЗОУИТ

        Args:
            text: Строка для проверки

        Returns:
            bool: True если формат КН или ЗОУИТ корректен
        """
    ...
    @staticmethod
    def get_zouit_field_name(layer: QgsVectorLayer) -> Optional[str]:
        """
        Определить имя поля номера ЗОУИТ в слое

        Проверяет наличие полей 'reg_numb_border' (WFS) или 'numb_border' (KPT-импорт)

        Args:
            layer: Слой для проверки

        Returns:
            str: Имя поля или None если не найдено
        """
        if layer.fields().indexFromName(CadnumSearchManager.FIELD_REG_NUMB_BORDER) >= 0:
            return CadnumSearchManager.FIELD_REG_NUMB_BORDER
        elif layer.fields().indexFromName(CadnumSearchManager.FIELD_NUMB_BORDER) >= 0:
            return CadnumSearchManager.FIELD_NUMB_BORDER
        return None

    @staticmethod
    def get_layers_with_zouit_field() -> List[QgsVectorLayer]:
        """
        Получить все векторные слои проекта с полем номера ЗОУИТ

        Ищет слои с полями 'reg_numb_border' или 'numb_border'

        Returns:
            List[QgsVectorLayer]: Список слоев с полем номера ЗОУИТ
        """
        layers = []

        for layer in QgsProject.instance().mapLayers().values():
            if isinstance(layer, QgsVectorLayer) and layer.isValid():
                if CadnumSearchManager.get_zouit_field_name(layer) is not None:
                    layers.append(layer)

        log_info(f"M_16: Найдено {len(layers)} слоев с полем номера ЗОУИТ")
        return layers
    ...
    @staticmethod
    def search_zouit_in_layer(layer: QgsVectorLayer, zouit_numbers: List[str]) -> Tuple[List[QgsFeature], List[str]]:
        """
        Поиск объектов ЗОУИТ в слое по списку номеров

        Args:
            layer: Слой для поиска
            zouit_numbers: Список номеров ЗОУИТ
        ...
        field_name = CadnumSearchManager.get_zouit_field_name(layer)
        if not field_name:
            return found_features, found_numbers

        formatted_values = ', '.join([QgsExpression.quotedValue(n) for n in zouit_numbers])
        expression = f'"{field_name}" IN ({formatted_values})'
        ...
        try:
            ...
            for feature in layer.selectedFeatures():
                found_features.append(feature)
                numb = feature.attribute(field_name)
                if numb:
                    found_numbers.append(str(numb))

            log_info(f"M_16: В слое {layer.name()} найдено {len(found_features)} ЗОУИТ объектов")

        except Exception as e:
            log_error(f"M_16: Ошибка поиска ЗОУИТ в слое {layer.name()}: {str(e)}")
...
class CadnumSearchDialog(BaseResponsiveDialog):
    ...
    def __init__(self, iface, parent=None):
        ...
        self.highlight_timer.timeout.connect(self.highlight_invalid_identifiers)

        self.init_ui()

        log_info("M_16: Диалог поиска создан")
    ...
    def init_ui(self):
        ...
        # === Секция ввода ===
        input_label = QLabel("Введите кадастровые номера или номера ЗОУИТ:")
        ...
        format_label = QLabel(
            "КН: Регион(1-2):Район(1-2):Квартал(4-7):Участок(1+)  |  "
            "ЗОУИТ: Регион(1-2):Район(1-2)-Номер.Подномер"
        )
        ...
        self.results_table.setHorizontalHeaderLabels(["Номер (КН / ЗОУИТ)", "Слой", "Статус"])
    ...
    def highlight_invalid_identifiers(self):
        """Подсветка невалидных номеров (КН и ЗОУИТ) красным цветом"""
        text = self.input_text.toPlainText()
        identifiers = CadnumSearchManager.parse_input(text)

        # Формат для невалидных номеров
        invalid_format = QTextCharFormat()
        invalid_format.setForeground(QColor(220, 50, 50))
        invalid_format.setFontWeight(QFont.Weight.Bold)
        invalid_format.setBackground(QColor(255, 200, 200, 50))  # Светло-красный фон

        # Список выделений для применения
        extra_selections = []

        # Подсвечиваем невалидные (не КН и не ЗОУИТ)
        for identifier in identifiers:
            if not CadnumSearchManager.validate_identifier(identifier):
                # Ищем все вхождения этого номера в тексте
                cursor = QTextCursor(self.input_text.document())
                cursor.setPosition(0)

                while True:
                    cursor = self.input_text.document().find(identifier, cursor)
                    if cursor.isNull():
                        break
    ...
    def perform_search(self):
        """Выполнить поиск по введенным номерам (КН и ЗОУИТ)"""
        text = self.input_text.toPlainText().strip()

        if not text:
            QMessageBox.warning(self, "Предупреждение", "Введите хотя бы один номер")
            return
        ...
        if not identifiers:
            QMessageBox.warning(self, "Предупреждение", "Не удалось распознать номера")
            return

        # Классификация: разделяем на КН, ЗОУИТ и невалидные
        valid_cadnums = []
        valid_zouit = []
        self.invalid_cadnums = []

        for ident in identifiers:
            id_type = CadnumSearchManager.classify_identifier(ident)
            if id_type == 'cadnum':
                valid_cadnums.append(ident)
            elif id_type == 'zouit':
                valid_zouit.append(ident)
            else:
                self.invalid_cadnums.append(ident)
        ...
        all_valid = valid_cadnums + valid_zouit
        if not all_valid:
            QMessageBox.warning(self, "Ошибка", "Не найдено ни одного валидного номера")
            return
        ...
        # Поиск КН в слоях с полем cad_num/cad_number
        if valid_cadnums:
            cadnum_layers = CadnumSearchManager.get_layers_with_cadnum()
            log_info(f"M_16: Поиск {len(valid_cadnums)} КН в {len(cadnum_layers)} слоях")

            for layer in cadnum_layers:
                features, found = CadnumSearchManager.search_in_layer(layer, valid_cadnums)
                if features:
                    if layer in self.search_results:
                        self.search_results[layer].extend(features)
                    else:
                        self.search_results[layer] = features
                    self.found_cadnums.update(found)

        # Поиск ЗОУИТ в слоях с полем reg_numb_border/numb_border
        if valid_zouit:
            zouit_layers = CadnumSearchManager.get_layers_with_zouit_field()
            log_info(f"M_16: Поиск {len(valid_zouit)} ЗОУИТ в {len(zouit_layers)} слоях")

            for layer in zouit_layers:
                features, found = CadnumSearchManager.search_zouit_in_layer(layer, valid_zouit)
                if features:
                    if layer in self.search_results:
                        self.search_results[layer].extend(features)
                    else:
        ...
        self.not_found_cadnums = [c for c in all_valid if c not in self.found_cadnums]

        # Отображаем результаты
        self.display_results(all_valid)
        ...
        # Активируем кнопку НСПД только если есть не найденные КН (не ЗОУИТ)
        not_found_cadnums_only = [c for c in self.not_found_cadnums
                                   if CadnumSearchManager.classify_identifier(c) == 'cadnum']
        self.nspd_button.setEnabled(len(not_found_cadnums_only) > 0)

        if not self.search_results and not self.not_found_cadnums:
            QMessageBox.information(self, "Результат",
                                   f"Ничего не найдено из {len(all_valid)} номеров")

    def display_results(self, searched_identifiers: List[str]):
        """
        Отображение результатов поиска в таблице

        Args:
            searched_identifiers: Список искомых номеров (КН и ЗОУИТ)
        """
        ...
        for layer, features in self.search_results.items():
            ...
            # Определяем имя поля: сначала КН, потом ЗОУИТ
            cadnum_field = CadnumSearchManager.get_cadnum_field_name(layer)
            zouit_field = CadnumSearchManager.get_zouit_field_name(layer)

            for feature in features:
                # Определяем значение из подходящего поля
                identifier = None
                if cadnum_field:
                    val = feature.attribute(cadnum_field)
                    if val and str(val) in searched_identifiers:
                        identifier = str(val)
                if not identifier and zouit_field:
                    val = feature.attribute(zouit_field)
                    if val and str(val) in searched_identifiers:
                        identifier = str(val)

                if not identifier:
                    # Fallback: попробовать оба поля
                    if cadnum_field:
                        identifier = str(feature.attribute(cadnum_field) or '')
                    elif zouit_field:
                        identifier = str(feature.attribute(zouit_field) or '')

                self.results_table.insertRow(row)

                # Номер
                item_cadnum = QTableWidgetItem(identifier or '-')
        ...
        for ident in self.not_found_cadnums:
            self.results_table.insertRow(row)

            # Номер
            item_cadnum = QTableWidgetItem(ident)
        ...
        total_searched = len(searched_identifiers)
    ...
    def clear_results(self):
        ...
        log_info("M_16: Результаты очищены")

    def fetch_from_nspd(self):
        """Загрузка геометрии не найденных КН из НСПД (только кадастровые номера, не ЗОУИТ)"""
        # Фильтруем только КН (ЗОУИТ не ищутся в НСПД)
        not_found_cadnums_only = [c for c in self.not_found_cadnums
                                   if CadnumSearchManager.classify_identifier(c) == 'cadnum']
        if not not_found_cadnums_only:
            QMessageBox.information(self, "Информация", "Нет не найденных кадастровых номеров для загрузки из НСПД")
            return

        # Подтверждение
        count = len(not_found_cadnums_only)
        ...
        try:
            for cadnum in not_found_cadnums_only:
            ...
            # Восстанавливаем кнопку (НСПД только для КН, не ЗОУИТ)
            self.nspd_button.setText("Загрузить из НСПД")
            remaining_cadnums = [c for c in self.not_found_cadnums
                                  if CadnumSearchManager.classify_identifier(c) == 'cadnum']
            self.nspd_button.setEnabled(len(remaining_cadnums) > 0)
