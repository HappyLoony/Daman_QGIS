...
class HardwareIDGenerator:
    ...
    def _collect_components(self) -> Dict[str, str]:
        ...
        # CPU ID (используем алиас 'cpu' вместо 'Win32_Processor' для совместимости)
        cpu_id = self._get_wmi_value("cpu", "ProcessorId")
        if cpu_id:
            components["cpu_id"] = cpu_id

        # Motherboard Serial (алиас 'baseboard')
        mb_serial = self._get_wmi_value("baseboard", "SerialNumber")
        if mb_serial and mb_serial not in ["To Be Filled By O.E.M.", "Default string", ""]:
            components["motherboard_serial"] = mb_serial

        # BIOS Serial (алиас 'bios')
        bios_serial = self._get_wmi_value("bios", "SerialNumber")
    ...
    def _get_wmi_value(self, wmi_class: str, property_name: str) -> Optional[str]:
        ...
        """
        try:
            # Using argument list instead of shell=True for security
            result = subprocess.run(
                ['wmic', wmi_class, 'get', property_name],
                capture_output=True,
                text=True,
                encoding='utf-8',
                errors='replace',  # WMIC может выводить в cp866/cp1251
                shell=False,
                timeout=10,
            ...
            if result.returncode == 0:
                # WMIC выводит: заголовок, пустые строки, значение
                # Фильтруем пустые строки и берём первое непустое значение после заголовка
                lines = [line.strip() for line in result.stdout.strip().split('\n') if line.strip()]
                if len(lines) >= 2:
                    value = lines[1]  # Первая непустая строка после заголовка
                    if value:
                        return value
