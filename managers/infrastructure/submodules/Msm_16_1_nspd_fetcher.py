...
from Daman_QGIS.constants import DEFAULT_MAX_RETRIES, NSPD_SEARCH_URL
...
class Msm_16_1_NspdFetcher:
    ...
    def _get_auth_cookies(self) -> dict:
        """Получить cookies авторизации НСПД (если доступны).

        Returns:
            dict cookies или пустой dict
        """
        try:
            from Daman_QGIS.managers import registry
            nspd_auth = registry.get('M_40')
            if nspd_auth and nspd_auth.is_authenticated():
                return nspd_auth.get_cookies()
        except Exception:
            pass
        return {}
    ...
    def fetch_by_cadnum(self, cadnum: str) -> NspdSearchResult:
        ...
        # Cookies авторизации НСПД (если пользователь авторизован)
