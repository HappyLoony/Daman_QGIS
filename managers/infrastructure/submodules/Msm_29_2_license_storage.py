...
Шифрование: AES-256-GCM (cryptography обязательна, устанавливается через F_4_1).
"""
...
# Ленивый импорт cryptography - библиотека устанавливается через F_4_1
if TYPE_CHECKING:
...
class LicenseStorage:
    ...
    def _derive_encryption_key(self) -> bytes:
        ...
        Threat Model:
        - ЗАЩИЩАЕТ: данные при копировании на другую машину (key derivation fails)
        - НЕ ЗАЩИЩАЕТ: от локального атакующего на той же машине (все inputs доступны)
        - НЕ ЗАЩИЩАЕТ: от целенаправленной атаки (алгоритм публичен)

        Это defense-in-depth, не security boundary.
        Основная защита - server-side валидация.
        """
        import platform
        import getpass
    ...
    def save_api_key(self, api_key: str):
        ...
        self._data["activated_at"] = datetime.now(timezone.utc).isoformat()
        self._save()
    ...
    def has_valid_cache(self) -> bool:
        ...
        Если последняя успешная проверка была менее 1 часа назад,
        разрешаем работу при недоступности сервера.
        """
