# -*- coding: utf-8 -*-
"""
Msm_26_2 - Маппинг и генерация атрибутов для нарезки ЗПР
...
class Msm_26_2_AttributeMapper:
    ...
    ZU_FIELD_MAPPING = {
        ...
        'ОКС_на_ЗУ_выписка': 'ОКС_на_ЗУ_выписка',
        'ОКС_на_ЗУ_факт': 'ОКС_на_ЗУ_факт',
    }
    ...
    def fill_generated_fields(
        ...
        # ID - явный или автоинкремент по имени слоя
        ...
        # Поля, заполняемые менеджерами позже (M_36, M_22)
        # setdefault: НЕ затираем если уже установлено
        ...
        # M_23.analyze_features_batch() может пересчитать позже (если слой ОКС доступен)
        # setdefault: НЕ затираем значения из выборки, только устанавливаем default для НГС
        result.setdefault('ОКС_на_ЗУ_выписка', '-')
        result.setdefault('ОКС_на_ЗУ_факт', '-')

        # Поля наложений - defaults, перезаписываются в _cut_by_overlays()
        result['НП'] = "-"
        result['МО'] = "-"
        result['Лес'] = "-"
        result['Вода'] = "-"
