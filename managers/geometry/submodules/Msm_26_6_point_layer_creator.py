# -*- coding: utf-8 -*-
"""
Msm_26_6 - Создание точечных слоёв для нарезки ЗПР

Создаёт слои характерных точек (MultiPoint) с уникальной нумерацией.
Структура: ID, ID_Контура, X(геодезический), Y(геодезический)

Слои точек:
- Le_2_5_1_1_Т_Раздел_ЗПР_ОКС
- Le_2_5_1_2_Т_НГС_ЗПР_ОКС
- и аналогично для ЛО, ВО

Перенесено из Fsm_2_1_6_point_layer_creator
"""
...
class Msm_26_6_PointLayerCreator:
    ...
    POINT_LAYER_MAPPING = {
        ...
        'Le_2_1_1_2_НГС_ЗПР_ОКС': 'Le_2_5_1_2_Т_НГС_ЗПР_ОКС',
        'Le_2_1_1_3_Без_Меж_ЗПР_ОКС': 'Le_2_5_1_3_Т_Без_Меж_ЗПР_ОКС',
        'Le_2_1_1_4_ПС_ЗПР_ОКС': 'Le_2_5_1_4_Т_ПС_ЗПР_ОКС',
        # ПО
        'Le_2_1_2_1_Раздел_ЗПР_ПО': 'Le_2_5_2_1_Т_Раздел_ЗПР_ПО',
        'Le_2_1_2_2_НГС_ЗПР_ПО': 'Le_2_5_2_2_Т_НГС_ЗПР_ПО',
        'Le_2_1_2_3_Без_Меж_ЗПР_ПО': 'Le_2_5_2_3_Т_Без_Меж_ЗПР_ПО',
        'Le_2_1_2_4_ПС_ЗПР_ПО': 'Le_2_5_2_4_Т_ПС_ЗПР_ПО',
        # ВО
        'Le_2_1_3_1_Раздел_ЗПР_ВО': 'Le_2_5_3_1_Т_Раздел_ЗПР_ВО',
        'Le_2_1_3_2_НГС_ЗПР_ВО': 'Le_2_5_3_2_Т_НГС_ЗПР_ВО',
        'Le_2_1_3_3_Без_Меж_ЗПР_ВО': 'Le_2_5_3_3_Т_Без_Меж_ЗПР_ВО',
        'Le_2_1_3_4_ПС_ЗПР_ВО': 'Le_2_5_3_4_Т_ПС_ЗПР_ВО',
    }
    ...
    def create_point_layer(
        ...
        Args:
            layer_name: Имя слоя (например, "Le_2_5_1_1_Т_Раздел_ЗПР_ОКС")
        ...
        """
        if not points_data:
            log_warning(f"Msm_26_6: Нет данных точек для слоя {layer_name}")
            return None
        ...
        try:
            ...
            if not features:
                log_warning(f"Msm_26_6: Не удалось создать features для слоя {layer_name}")
                return None
            ...
            if not success:
                log_error(f"Msm_26_6: Ошибка сохранения слоя {layer_name} в GPKG")
                return None
            ...
            if layer is not None and layer.isValid():
                log_info(f"Msm_26_6: Создан точечный слой {layer_name} ({layer.featureCount()} точек)")
                return layer
            else:
                log_error(f"Msm_26_6: Не удалось загрузить слой {layer_name} из GPKG")
                return None

        except Exception as e:
            log_error(f"Msm_26_6: Ошибка создания слоя {layer_name}: {e}")
            return None
    ...
    def _create_fields(self) -> QgsFields:
        ...
        - ID_Точки_контура: Номер точки внутри кольца (1, 2, 3... для каждого кольца)
        - ID_Контура: ID контура к которому принадлежит точка
        - Тип_кольца: 'exterior' для внешнего, 'hole' для внутреннего (дырки)
        - Индекс_кольца: 0 для внешнего, 1+ для дырок
        ...
        # ID точки внутри кольца (локальный номер: 1, 2, 3...)
        fields.append(QgsField("ID_Точки_контура", QMetaType.Type.Int))

        # ID контура
        fields.append(QgsField("ID_Контура", QMetaType.Type.Int))

        # Тип кольца: exterior или hole
        fields.append(QgsField("Тип_кольца", QMetaType.Type.QString))

        # Индекс кольца: 0 для внешнего, 1+ для дырок
        fields.append(QgsField("Индекс_кольца", QMetaType.Type.Int))
    ...
    def _create_features(
        ...
        for data in points_data:
            ...
            # Порядок: ID, ID_Точки_контура, ID_Контура, Тип_кольца, Индекс_кольца, X, Y
            feature.setAttributes([
                data.get('id', 0),
                data.get('contour_point_index', 0),  # ID_Точки_контура
                data.get('contour_id', 0),
                data.get('ring_type', 'exterior'),  # Тип_кольца
                data.get('ring_index', 0),  # Индекс_кольца
    ...
    def _save_to_gpkg(
        ...
        try:
            ...
            if not mem_layer.isValid():
                log_error(f"Msm_26_6: Не удалось создать memory layer для {layer_name}")
                return False
            ...
            if error[0] != QgsVectorFileWriter.WriterError.NoError:
                log_error(f"Msm_26_6: Ошибка записи в GPKG: {error[1]}")
                return False

            return True

        except Exception as e:
            log_error(f"Msm_26_6: Исключение при сохранении в GPKG: {e}")
            return False
    ...
    def _load_layer_from_gpkg(self, layer_name: str) -> Optional[QgsVectorLayer]:
        ...
        if layer.isValid():
            ...
            log_error(f"Msm_26_6: Не удалось загрузить слой из {uri}")
            return None
    ...
    def create_point_layer_for_polygon(
        ...
        if not point_layer_name:
            log_warning(f"Msm_26_6: Нет соответствия для полигонального слоя {polygon_layer_name}")
            return None
