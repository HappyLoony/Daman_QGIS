...
from qgis.core import (
    Qgis, QgsVectorLayer, QgsFeature, QgsGeometry, QgsPointXY, QgsWkbTypes
)
from Daman_QGIS.constants import PRECISION_DECIMALS, COORDINATE_TOLERANCE, CLOSURE_TOLERANCE
from Daman_QGIS.utils import log_info, log_warning

__all__ = ['CoordinatePrecisionManager']
...
class CoordinatePrecisionManager:
    ...
    def _round_geometry(geometry: QgsGeometry) -> Optional[QgsGeometry]:
        ...
        # makeValid() может вернуть GeometryCollection с артефактами коллапса
        # (LineString из Polygon, Point из LineString при snappedToGrid).
        # Аналог PostGIS ST_CollectionExtract: извлекаем части исходного типа.
        if rounded_geometry.wkbType() == QgsWkbTypes.GeometryCollection:
            source_geom_type = QgsWkbTypes.geometryType(geometry.wkbType())
            matching_parts = []
            for i in range(rounded_geometry.constGet().numGeometries()):
                part = rounded_geometry.constGet().geometryN(i)
                if QgsWkbTypes.geometryType(part.wkbType()) == source_geom_type:
                    matching_parts.append(QgsGeometry(part.clone()))

            if matching_parts:
                if len(matching_parts) == 1:
                    rounded_geometry = matching_parts[0]
                else:
