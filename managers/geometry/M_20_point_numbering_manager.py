...
- СЗ точка определяется как ближайшая к СЗ углу MBR кольца
- CW проверяется через Shoelace formula (signed_area < 0 в мат. СК)
"""
...
def _get_cpm():
    ...
    return CoordinatePrecisionManager
    ...
    def process_polygon_layer(
        ...
        Args:
            ...
            auto_reset: Сбросить счётчик перед обработкой (default=True).
                       ...
                       (счётчик и _unique_points сохраняются между вызовами).
                       Сейчас потребителей False нет: эталон M_26 нумерует
                       Раздел и НГС независимо; сквозная нумерация Раздел→НГС
                       в F_2_4 отложена (см. PLAN_F_2_4_point_numbering_unified).
    ...
    def _extract_polygon_points(
        ...
        for polygon in polygons:
            ...
            for ring in polygon:
                ...
                ring_points = ring
                if len(ring) > 1:
                    first_rounded = _get_cpm().round_point_tuple(ring[0], precision)
                    last_rounded = _get_cpm().round_point_tuple(ring[-1], precision)
                    if first_rounded == last_rounded:
                        ring_points = ring[:-1]

                for point in ring_points:
                    point_tuple = _get_cpm().round_point_tuple(point, precision)
        ...
        for polygon in polygons:
            ...
            for ring_idx, ring in enumerate(polygon):
                ...
                if points and len(points) >= 3:
                    is_exterior = (contour_type == 'Внешний')
                    points = self.normalize_ring(points, is_exterior=is_exterior)
