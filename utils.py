...
# ============================================================================
# ФИЛЬТР ЭКСПОРТНЫХ ПОЛЕЙ (транзитные __-поля)
# ============================================================================


# Префикс внутренних (транзитных) полей, исключаемых из ЛЮБОГО экспорта
# атрибутов. Носитель формы собственности `__Форма_тех` (Вариант B: форма
# транзитна — вход классификатора Msm_25_2, не выходная колонка). Живёт в
# .gpkg на время обработки, в выходные файлы (DXF/TAB/GeoJSON/KML/Shapefile/
# ведомость Excel) НЕ попадает. Конвенция greenfield — других `__`-полей нет.
INTERNAL_FIELD_PREFIX = "__"


def is_internal_field(field_name: Optional[str]) -> bool:
    """Проверка, что поле — внутреннее (транзитное), исключаемое из экспорта.

    КРИТИЧНО: сравнение по СЫРОМУ имени поля (`field.name()`), БЕЗ прогона
    через `Msm_13_1.sanitize_filename` — та схлопывает `__` -> `_` (функция
    для имён файлов/слоёв, не полей) и фильтр бы промахнулся (INST-1).

    Args:
        field_name: Имя поля (сырое, как в слое)

    Returns:
        True если имя начинается с `__` (транзитное поле)
    """
    return bool(field_name) and field_name.startswith(INTERNAL_FIELD_PREFIX)


def exportable_fields(layer) -> List:
    """Список полей слоя БЕЗ транзитных `__`-полей (для экспорта атрибутов).

    Единый хелпер фильтрации экспортных полей. Итерирующие экспортёры (DXF,
    TAB GDAL-ветка) фильтруют список через него; QGIS-writer тракты
    (GeoJSON/KML/Shapefile/TAB WGS-ветка через `writeAsVectorFormatV3`)
    берут индексы через `exportable_field_indices()` в `SaveVectorOptions.attributes`.

    Args:
        layer: QgsVectorLayer

    Returns:
        List[QgsField]: поля слоя без `__`-префикса (в исходном порядке)
    """
    return [f for f in layer.fields() if not is_internal_field(f.name())]


def exportable_field_indices(layer) -> List[int]:
    """Индексы экспортируемых полей слоя (без транзитных `__`-полей).

    Для передачи в `QgsVectorFileWriter.SaveVectorOptions.attributes` —
    список индексов полей, которые попадут в выходной файл. Пустой список
    означал бы «все поля», поэтому вызывающий код обязан всегда ставить
    результат (даже если `__`-полей нет — тогда это все поля явно).

    Args:
        layer: QgsVectorLayer

    Returns:
        List[int]: индексы полей без `__`-префикса (в исходном порядке)
    """
    return [
